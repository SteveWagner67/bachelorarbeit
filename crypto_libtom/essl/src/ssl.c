...
static const uint16_t rai_tlsSHAlgs[]       = {
		((GCI_HASH_SHA256 << 8) + (GCI_SIGN_RSA & 0xFF)),
		((GCI_HASH_MD5 << 8) + (GCI_SIGN_RSA & 0xFF)),
		((GCI_HASH_SHA1 << 8) + (GCI_SIGN_RSA & 0xFF)),
};
		...
		uint8_t*    pc_encSign,   size_t sz_ecnSignLen)
{
	...
	if (ps_sslCtx->s_secParams.s_signAlg.c_hash == GCI_HASH_INVALID)
	{
		...
		err = gci_hash_new_ctx(GCI_HASH_SHA1, &sha1Ctx);
...
//			}
			...
			err = gci_sign_verify_new_ctx(&rsaConf, ps_hsElem->gci_rsaCliPubKey, &rsaCtx);
			if (err != GCI_OK)
			{
				//TODO: return from error state
			}
		...
		uint8_t* pc_out,size_t* sz_outLen)
{
	...
	if (c_hashType == GCI_HASH_INVALID)
	{
		...
		//OLD-CW: cr_digestInit(&cwt_md5Ctx, NULL, 0, E_SSL_HASH_MD5);
		err = gci_hash_new_ctx(GCI_HASH_MD5, &md5Ctx);
		if (err != GCI_OK)
		{
			//TODO: return from error state
		}
		...
		//OLD-CW: cr_digestInit(&cwt_sha1Ctx, NULL, 0, E_SSL_HASH_SHA1);
		err = gci_hash_new_ctx(GCI_HASH_SHA1, &shaCtx);
		if (err != GCI_OK)
		{
			//TODO: return from error state
		}
	...
	case GCI_SIGN_RSA:
	{
		...
		signConf.config.rsa.padding = GCI_PADDING_PKCS1;
...
static void loc_hash(e_hashOp_t e_hashOp, s_sslCtx_t* ps_sslCtx, uint8_t* pc_in, size_t sz_inLen)
{
	...
	if (ps_sslCtx->s_secParams.e_prf == E_SSL_PRF_MD5_SHA1)
	{
		...
		case E_HASHOP_INIT:
			...
			err = gci_hash_new_ctx(GCI_HASH_MD5, &ps_sslCtx->ps_hsElem->u_hashCtx.md5Ctx);
			if (err != GCI_OK)
			{
				//TODO: return from error state
			}
			...
			err = gci_hash_new_ctx(GCI_HASH_SHA1, &ps_sslCtx->ps_hsElem->u_hashCtx.sha1Ctx);
			if (err != GCI_OK)
			{
				//TODO: return from error state
			}
		...
		uint8_t* pc_out, size_t cwt_outLen)
{
	...
	while (i_bytes > 0)
	{
		...
		err = gci_hash_update(sha1Ctx, ac_mix, c_mixCount);
		if (err != GCI_OK)
		{
			//TODO: return from error state
		}
		...
		err = gci_hash_update(sha1Ctx, pc_sec, cwt_secLen);
		if (err != GCI_OK)
		{
			//TODO: return from error state
		}
		...
		err = gci_hash_update(sha1Ctx, pc_par1, cwt_par1Len);
		if (err != GCI_OK)
		{
			//TODO: return from error state
		}
		...
		err = gci_hash_update(sha1Ctx, pc_par2, cwt_par2Len);
		if (err != GCI_OK)
		{
			//TODO: return from error state
		}
		...
		err = gci_hash_finish(sha1Ctx, sha1_hash, NULL);
		if (err != GCI_OK)
		{
			//TODO: return from error state
		}
		...
		err = gci_hash_update(md5Ctx, pc_sec, cwt_secLen);
		if (err != GCI_OK)
		{
			//TODO: return from error state
		}
		...
		err = gci_hash_finish(md5Ctx, pc_out, NULL);
		if (err != GCI_OK)
		{
			//TODO: return from error state
		}
		...
		size_t cwt_len)
{
	...
	case E_SSL_3_0:
		 ...
		 * about this   */
		if (ps_sslCtx->s_secParams.e_kst != GCI_KEY_PAIR_DH)
		{
...
static void loc_compKey(s_sslCtx_t * ps_sslCtx, uint8_t b_srvKey)
{
	...
	if (b_srvKey == TRUE)
	{
		...
		//OLD-CW: memcpy(ps_secPar->ac_srvSecret,&(ac_keyBlk[c_macOff]), ps_secPar->c_hmacLen);
		memcpy(ps_secPar->ac_srvSecret,&(ac_keyBlk[c_macOff]), ps_secPar->c_hmacLen);
	} /* if */
	else
	{
		...
		memcpy(ps_secPar->ac_cliSecret,&(ac_keyBlk[c_macOff]), ps_secPar->c_hmacLen);
	...
	case TLS_RSA_WITH_RC4_128_SHA:
	{
		...
		//No padding and block mode available for the RC4
		ciphConf.algo = GCI_CIPH_RC4;
		ciphConf.blockMode = GCI_BLOCK_MODE_INVALID;
		ciphConf.padding = GCI_PADDING_INVALID;
	...
	case TLS_ECDHE_RSA_WITH_3DES_EDE_CBC_SHA: //vpy
	{
		...
		ciphConf.algo = GCI_CIPH_TDES;
		ciphConf.blockMode = GCI_BLOCK_MODE_CBC;
		...
		e_sslCipSpec_t     e_cipSpec)
{
	...
	{
	case TLS_RSA_WITH_RC4_128_MD5:
		ps_secPar->e_kst            = GCI_KEY_PAIR_RSA;
		ps_secPar->s_signAlg.c_sign = GCI_SIGN_RSA;
		ps_secPar->e_cipType        = GCI_CIPH_RC4;
		ps_secPar->b_isBlkCip       = FALSE;
		ps_secPar->e_hmacType       = GCI_HASH_MD5;
	...
	case TLS_RSA_WITH_AES_128_CBC_SHA:
		ps_secPar->e_kst            = GCI_KEY_PAIR_RSA;
		ps_secPar->s_signAlg.c_sign = GCI_SIGN_RSA;
		ps_secPar->e_cipType        = GCI_CIPH_AES;
		ps_secPar->b_isBlkCip       = TRUE;
	...
	case TLS_RSA_WITH_AES_128_CBC_SHA256:
		...
		ps_secPar->b_isBlkCip       = TRUE;
		ps_secPar->c_blockLen       = 16;
		ps_secPar->e_hmacType       = GCI_HASH_SHA256;
		ps_secPar->c_keyLen         = 16;
		break;
	case TLS_RSA_WITH_AES_256_CBC_SHA:
		ps_secPar->e_kst            = GCI_KEY_PAIR_RSA;
	...
	case TLS_RSA_WITH_AES_256_CBC_SHA256:
		...
		break;
	case TLS_DHE_RSA_WITH_3DES_EDE_CBC_SHA:
		ps_secPar->e_kst            = GCI_KEY_PAIR_DH;
		ps_secPar->s_signAlg.c_sign = GCI_SIGN_RSA;
		ps_secPar->e_cipType        = GCI_CIPH_TDES;
		ps_secPar->b_isBlkCip       = TRUE;
		ps_secPar->c_blockLen       = 8;
	...
	case TLS_ECDHE_ECDSA_WITH_3DES_EDE_CBC_SHA:
		ps_secPar->e_kst            = GCI_KEY_PAIR_ECDH;
...
#endif
	default:
		ps_secPar->e_kst            = GCI_KEY_PAIR_INVALID;
		ps_secPar->s_signAlg.c_sign = GCI_SIGN_INVALID;
		ps_secPar->e_cipType        = GCI_CIPH_INVALID;
		ps_secPar->b_isBlkCip       = FALSE;
		ps_secPar->c_blockLen       = 0;
		ps_secPar->e_hmacType       = GCI_HASH_INVALID;
...
//			//end vpy
			...
			default:
				ps_secPar->s_signAlg.c_sign = GCI_SIGN_INVALID;
				ps_secPar->s_signAlg.c_hash = GCI_HASH_INVALID;
			}
		}
		...
		uint16_t i_cipSpecListLen, uint16_t u_bufLen)
{
	...
	*pe_selCip = TLS_NULL_WITH_NULL_NULL;
	...
	if ((pc_ret != NULL) || (l_inc == 3))
	{
		...
		if (ps_sslCtx->e_ver == E_TLS_1_2) {
			ps_sslCtx->s_secParams.s_signAlg.c_hash = GCI_HASH_SHA1;
		} else {
			/* For all other versions it's assumed that sha1 + md5 are
			 * supported */
			ps_sslCtx->s_secParams.s_signAlg.c_hash = GCI_HASH_INVALID;
		}
	}
	/*
		...
		case TLS_EXTENSION_RENEGOTIATION_INFO:
		{
			...
			/* So this is a re-negotiation */
						...
						(b_isCli && (*pc_curExt != (l_lvdLen * 2))))
				{
					 ...
					 */
					if (memcmp(pc_curExt + 1, ps_g->ac_cliVerifyData, l_lvdLen) != 0)
					{
						/*
	...
	if(ps_sslCtx->b_isCli) //is client ?
	{
		...
		if ((ps_sslCtx->c_isRenegOn == TRUE) && (ps_sslCtx->c_secReneg == TRUE))
		{
			...
			else
			{
				 ...
				 */
				memcpy(pc_out, &ps_sslCtx->s_sslGut.ac_cliVerifyData[0],l_outLen);
				pc_out += l_outLen;
				/*
				...
				if (ps_sslCtx->b_isCli == FALSE)
				{
					 ...
					 */
					memcpy(pc_out, &ps_sslCtx->s_sslGut.ac_srvVerifyData[0], l_outLen);
					pc_out += l_outLen;
					/*
		...
		if ((ps_sslCtx->e_ver == E_TLS_1_2) &&
				...
				(ps_sslCtx->ps_sslSett->s_certSignHashAlg.c_hash != GCI_HASH_INVALID))
		{
			uint16_t    i_signHashAlg;
			uint8_t     i;
			uint8_t     c_hash = GCI_HASH_INVALID;
		...
		*pcwt_recLen = 2;
		...
		case E_SSL_SM_SEND_CLIENT_HELLO:
			...
			case E_SSL_ASM_STEP1:
				LOG1_INFO("%p| SEND_CLIENT_HELLO ASM_STEP1 (ClientHello)",
						ps_sslCtx);
				memcpy(pc_rec, (void*)rac_cliHello,
						sizeof(rac_cliHello));
		...
		case E_SSL_SM_SEND_SERVER_HELLO:
				...
				case E_SSL_ASM_START: /* Prepare Server Hello */
					...
					memcpy(pc_rec, (void*)rac_srvHello,
							sizeof(rac_srvHello));
				...
				case E_SSL_ASM_STEP2:
				{
					...
					case GCI_KEY_PAIR_ECDH:
						...
						dhConf.type = GCI_ECDH;
						//TODO sw - fix a curve intern


						err = gci_dh_new_ctx(&dhConf, &ps_secPar->eccCtx);
						if(err != GCI_OK)
						{
							//TODO return an error
						}
					...
					default:
						...
						err = gci_key_get(ps_secPar->dhePeerPubKey, &dhSrvPubKey);
						if(err != GCI_OK)
						{
				...
				case E_SSL_ASM_STEP4:
					LOG1_INFO("%p| SEND_SERVER_HELLO ASM_STEP4 (SrvHelloDone)",
							ps_sslCtx);
					memcpy(pc_rec, (void*)rac_srvHelloDone,
			...
			case E_SSL_SM_SEND_CLIENT_FINISH:
					...
					case E_SSL_ASM_STEP1:
						...
						case GCI_KEY_PAIR_RSA:
							...
							err = gci_cipher_new_ctx(NULL, ps_hsElem->gci_rsaCliPubKey, &rsaCtx);
							if(err != GCI_OK)
							{
								//TODO return state from error
							}
						...
						case GCI_KEY_PAIR_DH:
							...
							err = gci_key_get(ps_hsElem->gci_dheSrvPubKey, &dhSrvPubKey);
							if(err != GCI_OK)
							{
								//return error state
							}
							...
							dhConf.config.dhDomain->g.len = dhSrvPubKey.key.dhPub.param->g.len;

							memcpy(dhConf.config.dhDomain->g.data,
								   dhSrvPubKey.key.dhPub.param->g.data,
								   dhSrvPubKey.key.dhPub.param->g.len);

							dhConf.config.dhDomain->p.len = dhSrvPubKey.key.dhPub.param->p.len;

							memcpy(dhConf.config.dhDomain->p.data,
									dhSrvPubKey.key.dhPub.param->p.data,
									dhSrvPubKey.key.dhPub.param->p.len);
							...
							err = gci_dh_gen_key(dhCtx, ps_hsElem->gci_dheCliPubKey);
							if(err != GCI_OK)
							{
								//TODO return state from error
							}
							...
							err = gci_dh_calc_sharedSecret(dhCtx, ps_hsElem->gci_dheSrvPubKey, &ps_hsElem->gci_dheSecKey);
							...
							err = gci_key_get(&ps_hsElem->gci_dheSecKey, &dhSecretKey);
...
//											ps_hsElem->s_sessElem.ac_msSec, MSSEC_SIZE);
							ps_sslCtx->e_lastError =
									loc_prf(ps_sslCtx,
											dhSecretKey.key.dhSecret.data, dhSecretKey.key.dhSecret.len,
											rac_TLSlabelMsSec, strlen((const char *)rac_TLSlabelMsSec),
											ps_hsElem->ac_cliRand, CLI_RANDSIZE,
											ps_hsElem->ac_srvRand, SRV_RANDSIZE,
											ps_hsElem->s_sessElem.ac_msSec, MSSEC_SIZE);
...
//											- (size_t) ps_sslCtx->ac_socBuf));
							...
							err = gci_key_get(ps_hsElem->gci_dheCliPubKey, &dhCliPubKey);
							if(err != GCI_OK)
							{
								//TODO return state from error
							}
						...
						case GCI_KEY_PAIR_ECDH:
							...
							err = gci_key_get(ps_hsElem->eccPubKeyPeer, &ecdhCliPubKey);
							if(err != GCI_OK)
							{
								//TODO return state from error
							}
							...
							ecdhConf.config.ecdhCurveName = ecdhCliPubKey.key.ecdhPub.curve;
							...
							err = gci_dh_calc_sharedSecret(ecdhCtx, ps_hsElem->eccPubKeyPeer, ps_hsElem->gci_ecdheSecKey);
							...
							//Get the big number of the secret key
							err = gci_key_get(ps_hsElem->gci_ecdheSecKey, &ecdhSecretKey);
							if(err != GCI_OK)
							{
								//TODO return state
							}
...
//											ps_hsElem->s_sessElem.ac_msSec, MSSEC_SIZE);
							...
							ps_sslCtx->e_lastError =
									loc_prf(ps_sslCtx,
											ecdhSecretKey.key.ecdhSecret.data, ecdhSecretKey.key.ecdhSecret.len,
											rac_TLSlabelMsSec, strlen((const char *)rac_TLSlabelMsSec),
											ps_hsElem->ac_cliRand, CLI_RANDSIZE,
											ps_hsElem->ac_srvRand, SRV_RANDSIZE,
											ps_hsElem->s_sessElem.ac_msSec, MSSEC_SIZE);
...
//													- (size_t) ps_sslCtx->ac_socBuf));
							...
							cwt_hashLen = (size_t) (sizeof(ps_sslCtx->ac_socBuf)
									- (ecdhSecretKey.key.ecdhSecret.len
											- (size_t) ps_sslCtx->ac_socBuf));
...
//							break;
						...
						case E_SSL_ASM_STEP4:
							...
							*pc_write++ = ps_sslGut->c_verifyDataLen;
							...
							memcpy(ps_sslGut->ac_cliVerifyData, pc_write, ps_sslGut->c_verifyDataLen);
							pc_write += ps_sslGut->c_verifyDataLen;
							cwt_hashLen = *pcwt_recLen = pc_write - pc_rec;
					...
					case E_SSL_SM_SEND_SERVER_FINISH:
						...
						case E_SSL_ASM_STEP1:
							...
							*pc_write++ = ps_sslCtx->s_sslGut.c_verifyDataLen;
							...
							memcpy(ps_sslCtx->s_sslGut.ac_srvVerifyData, pc_write, ps_sslCtx->s_sslGut.c_verifyDataLen);
							pc_write += ps_sslCtx->s_sslGut.c_verifyDataLen;
							cwt_hashLen = *pcwt_recLen = pc_write - pc_rec;
						...
						case E_SSL_SM_SEND_SERVER_HELLO_FINISH:
							...
							case E_SSL_ASM_STEP2:
								...
								*pc_write++ = ps_sslGut->c_verifyDataLen;
								...
								memcpy(ps_sslGut->ac_srvVerifyData, pc_write, ps_sslGut->c_verifyDataLen);
								pc_write += ps_sslGut->c_verifyDataLen;
								cwt_hashLen = *pcwt_recLen = pc_write - pc_rec;
	...
	case E_SSL_RT_HANDSHAKE:
	{
		...
		do
		{
			...
			case CLIENT_HELLO:
			{
				...
				if (cwt_len > 0)
				{
					memcpy(ps_hsElem->s_sessElem.ac_id, pc_hsBuff+39, cwt_len);
			...
			case SERVER_HELLO:
				...
				else
				{
					...
					if (ps_sslCtx->c_isResumed == TRUE)
					{
						if (memcmp(ps_hsElem->s_sessElem.ac_id,
								pc_hsBuff, cwt_len) != 0)
						{
				...
				case SERVER_KEY_EXCHANGE:
				{
					...
					case GCI_KEY_PAIR_ECDH:
						...
						eccPub.key.ecdhPub.coord.x.len = pc_hsBuff;

						pc_hsBuff++;

						memcpy(eccPub.key.ecdhPub.coord.x.data, pc_hsBuff, eccPub.key.ecdhPub.coord.x.len);

						pc_hsBuff+=eccPub.key.ecdhPub.coord.x.len;

						//the y-coordinate has a length of the rest of the half of the public key's length
						eccPub.key.ecdhPub.coord.y.len = pc_hsBuff;

						pc_hsBuff++;

						memcpy(eccPub.key.ecdhPub.coord.y.data, pc_hsBuff, eccPub.key.ecdhPub.coord.y.len);

						pc_hsBuff+=eccPub.key.ecdhPub.coord.y.len;
...
//						}
						...
						srvPub.key.dhPub.param->p.len = pc_hsBuff;
						pc_hsBuff++;

						//Copy the prime data
						memcpy(srvPub.key.dhPub.param->p.data, pc_hsBuff, srvPub.key.dhPub.param->p.len);
						pc_hsBuff+= srvPub.key.dhPub.param->p.len;

						//Read the length of the generator
						srvPub.key.dhPub.param->g.len = pc_hsBuff;

						pc_hsBuff++;

						//Copy the generator data
						memcpy(srvPub.key.dhPub.param->g.data, pc_hsBuff, srvPub.key.dhPub.param->g.len);
						pc_hsBuff+= srvPub.key.dhPub.param->g.len;

						//Read the server public key length
						srvPub.key.dhPub.key.len = pc_hsBuff;
						pc_hsBuff++;

						//Copy the server public key
						memcpy(srvPub.key.dhPub.key.data, pc_hsBuff, srvPub.key.dhPub.key.len);
...
//						pc_hsBuff += *pc_hsBuff * 256 + pc_hsBuff[1] + 2;
				...
				case CERTIFICATE_VERIFY:
					...
					if (ps_sslCtx->e_ver > E_TLS_1_1) {
						...
						memcpy(ps_hsElem->ac_hsBuf+VERIF_HASHSIZE, pc_hsBuff+6, cwt_len);
						*cwt_wDataLen = cwt_len + VERIF_HASHSIZE;
				...
				case FINISHED:
				{
...

					{
						...
						else
						{
							if (ps_sslCtx->b_isCli == TRUE)
							{
								memcpy(ps_sslGut->ac_srvVerifyData,
										pc_hsHash,
										ps_sslGut->c_verifyDataLen);
							}
							else
							{
								memcpy(ps_sslGut->ac_cliVerifyData,
										pc_hsHash,
										ps_sslGut->c_verifyDataLen);
							}
						}
		...
		s_sslHsElem_t *ps_sslHsElem)
{
	...
	ps_sslCtx->s_secParams.e_kst = GCI_KEY_PAIR_NONE;

	ps_sslCtx->s_secParams.s_signAlg.c_hash = GCI_HASH_INVALID;

	ps_sslCtx->s_secParams.s_signAlg.c_sign = GCI_SIGN_INVALID;
