...
%	Interface									 %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\node (prov) [block, below of=embtls, text width=6em,]{\tomcrypt};

\path (prov.south) +(-1.75,-0.4) node (int1) [text width=0.5]
{\small{Provider}};

\begin{pgfonlayer}{background}
          
   	\path (prov.west |- prov.north)+(-0.75,0.65) node (c) {};
   	\path (prov.south -|  prov.east)+(0.75,-0.65) node (d) {};
...
\path [draw, <->] (prov.north) -- (embtls.south);
