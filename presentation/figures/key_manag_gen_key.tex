...
\node (int) [rect] {Interface};

\path (int.west)+(-1,0) node (id) [txtblk] {\scriptsize{ID x: Key}};
\path (int.east)+(3,0) node (app) [rect] {Application};
\path (int.east)+(1,0) node (res) [txtblk] {\scriptsize{key ID x}};

\path (int.south)+(0,-2) node (prov) [rect] {Provider};
\path (int.south)+(0.35,-1) node (key) [txtblk] {\scriptsize{Key}};


\path [draw, ->] (int.east) -- (app.west);
\path [draw, <-] (int.south) -- (prov.north);
