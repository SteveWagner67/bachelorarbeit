...
int init_client_CA_certs(char ** ppc_CAcerts) {
	...
	while (i < SSL_CLIENT_CA_CERTS_NUM && (pc_CAcert = ppc_CAcerts[i]) != NULL) {
		...
		if (so_caCert.pc_data == NULL) {
			...
			//TODO vpy init only the type of key used
			c_ret = cw_rsa_publickey_init(&as_caCert[i].caPubKey.key.rsaPub);
			//c_ret |= cw_ecc_publickey_init(&as_caCert[i].caPubKey.key.eccPub);

			c_ret |= sslCert_init(	&so_caCert,
					&as_caCert[i],
					&as_caCert[i].caPubKey,
			...
			if (c_ret == E_SSL_CERT_OK) {
				...
				if((as_caCert[i].caPubKey.type==KEY_RSA_PRIV) || (as_caCert[i].caPubKey.type==KEY_RSA_PUB))
				{
					cw_rsa_publickey_shrink(&as_caCert[i].caPubKey.key.rsaPub);
				}
				...
				//cw_rsa_publickey_free(&as_caCert[i].caPubKey.key.rsaPub);
				cw_publickey_free(&as_caCert[i].caPubKey);
				c_ret = -1;
			} /* if ... else */
