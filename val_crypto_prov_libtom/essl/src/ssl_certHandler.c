...
static e_derdRet_t loc_getPubKeyInfo(s_derdCtx_t *ps_ctx, s_pubKey_t *ps_pubKey);
static e_derdRet_t loc_getECCPubKey(s_derdCtx_t *ps_ctx, s_pubKey_t *ps_pubKey);
 ...
 *
 *  RSAPublicKey ::= SEQUENCE {
 *      x(?)          INTEGERT,       --> cw_bigNum_t
 *      y(?)		  INTEGER }       --> cw_bigNum_t
 ****************************************************************************/
static e_derdRet_t loc_getECCPubKey(s_derdCtx_t *ps_ctx, s_pubKey_t *ps_pubKey)
{
	int res = E_SSL_DER_OK;
	...
	s_sslBitStr_t ps_temp;

	//Read the bit string containing the key
	res = sslDerd_getBitStr(ps_ctx, &ps_temp);
	if (res == E_SSL_DER_OK)
	{
		//Copy the length and the datas of the bit string into our pubKey structure

		//Copy the size
		ps_pubKey->uiKeyLen = ps_temp.cwt_len;

		//Allocate memory for the raw key //TODO vpy ECDSA: free this memory
		ps_pubKey->eccKeyRaw = (uint8_t*) malloc (ps_pubKey->uiKeyLen*(sizeof(uint8_t)));

		//Copy the raw key
		int i=0;
		for(i=0; i< ps_pubKey->uiKeyLen; i++)
		{
			ps_pubKey->eccKeyRaw[i] = ps_temp.pc_bitStr[i];
		}
	}
	else
	{
		res = E_SSL_DER_ERR_NO_ECC;
	}
	return res;
} /* loc_getECCPubKey */
 ...
 *      (ECC curve)			ECC curve identifier --> int
...
static e_derdRet_t loc_getPubKeyInfo(s_derdCtx_t *ps_ctx, s_pubKey_t *ps_pubKey)
{
            ...
            && (sslDerd_getNextValue(ps_ctx) == SSL_DER_ASN1_CSEQUENCE))
    {
        e_res = sslDerd_getPubKeyAlg(ps_ctx, &ps_pubKey->iAlgorithm,
                &ps_pubKey->uiKeyLen, &ps_pubKey->eccCurve);
        if (e_res == E_SSL_DER_OK)
        {
            if (sslDerd_getNextBitStr(ps_ctx) == E_SSL_DER_OK)
            {
                ...
                case SSL_OID_X509_RSA_ENC:
                    ...
                    break;
                case SSL_OID_EC_PUBLIC_KEY:
                	e_res = loc_getECCPubKey(ps_ctx, ps_pubKey);
                	break;
                default:
...
e_sslCertErr_t ssl_verifyCertSign(s_sslKeyCertInfo_t *ps_certInfo, Key_t *p_Key)
{
    int32_t l_hashAlgo;
    int32_t l_verifyRes;
    uint8_t ac_buf[CW_MAX_HASHSIZE];
    ...
    if(p_Key->type == KEY_ECC_PUB)// && p_Key.key.eccPub != NULL)
    {
    	printf("ECC verification\n"); //vpy debug
    	e_ret = 0;
    	e_ret = cw_ecc_hash_verify_ltc(ps_certInfo->s_sign.pc_bitStr, ps_certInfo->s_sign.cwt_len, ac_buf, ul_bufLen, /*l_hashAlgo,*/
                &l_verifyRes, &p_Key->key.eccPub);
    }
    else if(p_Key->type == KEY_RSA_PUB)
    {
    	printf("RSA verification\n"); //vpy debug
    	e_ret = cw_rsa_hash_verify_ltc(ps_certInfo->s_sign.pc_bitStr, ps_certInfo->s_sign.cwt_len, ac_buf, ul_bufLen, l_hashAlgo,
                &l_verifyRes, &p_Key->key.rsaPub);
    }
    else
    {
    ...
    printf("ssl_verifyCertSign: result: %d\n", e_ret);
    return (e_ret);
} /* ssl_verifyCertSign */
                               ...
                               s_sslOctetStr_t  *ps_pubKeyStr)
{
    ...
    else
    {
        ...
        e_res = loc_getPubKeyInfo(&s_derdCtx, ps_pubKeyInfo);
    }
