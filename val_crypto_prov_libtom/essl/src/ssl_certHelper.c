                            ...
                            s_sslCertList_t *ps_caListHead)
{
    ...
    else if (ps_caListHead != NULL)
    {
        ...
        //try to find the same certificate in CAlist (vpy)
            ...
            || ((ps_cert->c_isCa == FALSE) && (ps_tmpCaRootCert != NULL) && (ps_tmpCaRootCert->c_isCa == TRUE)))
    {
         ...
         */
    	//TODO vpy: ADD FOR ECDSA
        cw_rsa_publickey_prep(&p_PubKey->key.rsaPub, &s_pubKey);

        //Get public key infos: Agorithm and raw values
        ...
        if(s_pubKey.iAlgorithm==SSL_OID_EC_PUBLIC_KEY) //ECDSA
        {
        	p_PubKey->type = KEY_ECC_PUB;
        	cw_publickey_free(&p_PubKey->key.rsaPub);
        	cw_publickey_init(&p_PubKey->key.eccPub); //test
        	cw_ecc_publickey_post(&s_pubKey,  &p_PubKey->key.eccPub);
        }
        else //RSA
        {
        	p_PubKey->type = KEY_RSA_PUB;
        	//cw_publickey_init(&p_PubKey->key.rsaPub);
        	cw_rsa_publickey_post(&s_pubKey, &p_PubKey->key.rsaPub);

        	printf("New key read from cert:\n");

        	//debug vpy
        	char buffer[4096];
        	mp_toradix(p_PubKey->key.rsaPub.e, buffer, 16);
        	printf("e: %s\n", buffer);

        	mp_toradix(p_PubKey->key.rsaPub.d, buffer, 16);
        	printf("d: %s\n", buffer);

        	mp_toradix(p_PubKey->key.rsaPub.N, buffer, 16);
        	printf("N: %s\n", buffer);
        }
        ...
        if (ps_tmpCaRootCert != NULL)
            i_tmpRet = ssl_verifyCertSign(&s_certInfo, &ps_tmpCaRootCert->caPubKey);
...
e_sslResult_t sslCert_getSubject(s_sslCertList_t *ps_entry, uint8_t *pc_dest, size_t *pcwt_space)
{
    ...
    else if (ps_entry->ps_cdbCert != NULL)
    {
        ...
        if (s_octCert.pc_data != NULL)
        {
            ...
            //TODO vpy: ECDSA
            cw_rsa_publickey_init(&tmp_cert.caPubKey.key.rsaPub);
            i_ret = sslCert_init(&s_octCert, &tmp_cert, &tmp_cert.caPubKey, pc_dest, *pcwt_space,
            NULL, NULL);
             ...
             */
            //TODO vpy: ECDSA
            //cw_rsa_publickey_free(&tmp_cert.caPubKey.key.rsaPub);
			cw_publickey_free(&tmp_cert.caPubKey);
            /*
...
/*==============================================================================
 ...
 ==============================================================================*/
//TODO vpy: ECDSA
e_sslResult_t sslCert_verifyChain(s_sslOctetStr_t *ps_octInData, Key_t *p_PubKey,
        s_sslCertList_t *ps_caListHead)
{
    e_sslCertErr_t i_ret;
    //cw_rsaPubKey_t *pcwt_tmpRsaPubKey; //TODO vpy: ECDSA
    Key_t *p_tmpPubKey;
    ...
    *ps_octTbvStor = loc_allocSubjStor();
    ...
    //TODO vpy: ECDSA
    i_ret = sslCert_init(&s_octPeerCert, &s_peerCert, p_PubKey,
    ...
    if ((i_ret != E_SSL_CERT_OK) && ((ps_octInData->cwt_len - 6) > s_octPeerCert.cwt_len))
    {
         ...
         */
    	//TODO vpy: ECDSA
        cw_rsa_publickey_init(&ast_tmpCert[0].caPubKey.key.rsaPub);
        cw_rsa_publickey_init(&ast_tmpCert[1].caPubKey.key.rsaPub);
        ...
        *ps_octRoot = s_octPeerCert;
        ...
        do
        {
             ...
             */
            i_ret = sslCert_init(ps_octRoot, ps_rootCert, &ps_rootCert->caPubKey, ps_octRootStor->pc_data,
            ...
            if (i_ret != E_SSL_CERT_OK)
            {
                 ...
                 */
            	//TODO vpy: ECDSA
                //cw_rsa_publickey_free(&ps_rootCert->caPubKey.key.rsaPub);
            	cw_publickey_free(&ps_rootCert->caPubKey);
                cw_rsa_publickey_init(&ps_rootCert->caPubKey.key.rsaPub);
            }
            ...
            do
            {
                ...
                *ps_octTbv = s_octPeerCert;
                ...
                if ((ps_octTbv->pc_data + ps_octTbv->cwt_len + 3) == ps_octRoot->pc_data)
                {
                     ...
                     */
                    //TODO vpy: ECDSA
                    //pcwt_tmpRsaPubKey = &p_PubKey->key.rsaPub;
                    p_tmpPubKey = p_PubKey;
                }
                else
                {
                    /*
                     ...
                     */
                    //TODO vpy: ECDSA
                    //pcwt_tmpRsaPubKey = &ps_tbvCert->caPubKey.key.rsaPub;
                    p_tmpPubKey = &ps_tbvCert->caPubKey;
                    /*
         ...
         */
        //TODO vpy: ECDSA
        //cw_rsa_publickey_free(&ast_tmpCert[0].caPubKey.key.rsaPub);
        //cw_rsa_publickey_free(&ast_tmpCert[1].caPubKey.key.rsaPub);
        cw_publickey_free(&ast_tmpCert[0].caPubKey);
        cw_publickey_free(&ast_tmpCert[1].caPubKey);
...
/****************************************************************************/
uint8_t *sslCert_extractX509Elem(uint8_t *pc_read, cw_bigNum_t **ppcwt_bigNum)
{
    size_t elementLen;
    ...
    if (ppcwt_bigNum != NULL)
    {
        ...
        *ppcwt_bigNum = cw_bn_create(*ppcwt_bigNum, elementLen * 8);
        cw_rsa_os2ip(*ppcwt_bigNum, pc_read, elementLen); //TODO vpy: ECDSA
    }
