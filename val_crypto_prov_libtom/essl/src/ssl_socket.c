...
void sslSoc_initSett(s_sslSett_t* ps_sslSett, e_sslKeyType_t keyType)
{
	...
	//Choose sign algorithm in function of the type of key provided. Either ECDSA or RSA
	//TODO vpy ECDSA: sign hash should be selected later.
...
void sslSoc_freeSett(s_sslSett_t* ps_sslSett)
{
	...
	if (ps_sslSett->pcwt_rsaMyPrivKey)
	{
		cw_rsa_privatekey_free(ps_sslSett->pcwt_rsaMyPrivKey);
		free(ps_sslSett->pcwt_rsaMyPrivKey);
	} /* if */
...
int sslSoc_setRsaPrivKey(s_sslSett_t* ps_sslSett, s_cdbCert_t* pcdt_privKey)
{
	...
	ps_sslSett->pcwt_rsaMyPrivKey = malloc(sizeof(cw_rsaPrivKey_t));

	iRet = _sslSoc_sett_import_RSAprivKey(pcdt_privKey, ps_sslSett->pcwt_rsaMyPrivKey);
	if (iRet != E_SSL_OK)
	{
		ps_sslSett->pcwt_rsaMyPrivKey = NULL;
	}
...
int sslSoc_free(s_sslCtx_t* ps_sslCtx)
{
	...
	//cw_rsa_publickey_free(&ps_sslCtx->ps_hsElem->peerPubKey.key.rsaPub);
	cw_publickey_free(&ps_sslCtx->ps_hsElem->peerPubKey);

	cw_dh_free(&ps_sslCtx->ps_hsElem->cwt_dheCliPrivKey);

	cw_dh_free(&ps_sslCtx->ps_hsElem->cwt_dheSrvPubKey);

	if (ps_sslCtx->ps_hsElem->pcwt_dheP != NULL)
		cw_bn_freefree(ps_sslCtx->ps_hsElem->pcwt_dheP);
...
int sslSoc_accept(s_sslCtx_t * ps_sslCtx)
{
	...
	if ((ps_sslCtx->ps_sslSett->pcwt_rsaMyPrivKey != NULL)|| (ps_sslCtx->ps_sslSett->p_ECCMyPrivKey !=NULL)) //vpy
	{
		switch (ps_sslCtx->s_sslGut.e_smState)
		{
...
int sslSoc_connect(s_sslCtx_t* ps_sslCtx)
{
	...
	if (((ps_sslCtx->e_authLvl & E_SSL_MUST_AUTH) != E_SSL_MUST_AUTH)
			|| ((ps_sslCtx->ps_sslSett->ps_certChainListHead != NULL)
					&& (ps_sslCtx->ps_sslSett->pcwt_rsaMyPrivKey != NULL))) //TODO vpy add ECDSA key
	{
		if (ps_sslCtx->s_sslGut.e_smState == E_SSL_SM_APPDATA_EXCHANGE)
