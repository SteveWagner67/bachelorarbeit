...
/*  sslDerd_getBigNum()                                                         */
/*============================================================================*/
e_derdRet_t sslDerd_getBigNum(s_derdCtx_t *ps_derdCtx, cw_bigNum_t **ppcwt_val)
{
    e_derdRet_t e_res = E_SSL_DER_OK;
    size_t cwt_tmp = 0;
            ...
            && (sslDerd_getNextValue(ps_derdCtx) == SSL_DER_ASN1_OBJECT))
    {
        *pl_sigAlg = sslOid_fromDer(ps_derdCtx->s_octVal.pc_data,
        ...
        if ((*pl_sigAlg == SSL_OID_MD2_WITH_RSA_ENC)
        		...
        		//|| (*pl_sigAlg == SSL_OID_SHA256_WITH_ECDSA_ENC) //vpy
				)
        {
            /*
             *  for RSA with MD5 or SHA1 there are no algorithm parameters
        ...
        else if (*pl_sigAlg == SSL_OID_SHA256_WITH_ECDSA_ENC)
        {
        	//if (sslDerd_getNextValue(ps_derdCtx) != SSL_DER_ASN1_NULL)
        	//{
        		//e_res = E_SSL_DER_ERR_NO_ALGNULLPAR;
...
e_derdRet_t sslDerd_getPubKeyAlg(s_derdCtx_t *ps_derdCtx, int32_t *pl_sigAlg,
        uint32_t *pl_keyLen, uint16_t *p_eccCurve)
{
    e_derdRet_t e_res = E_SSL_DER_OK;
            ...
            && (sslDerd_getNextValue(ps_derdCtx) == SSL_DER_ASN1_OBJECT))
    {
        *pl_sigAlg = sslOid_fromDer(ps_derdCtx->s_octVal.pc_data,
        ...
        else if (*pl_sigAlg == SSL_OID_EC_PUBLIC_KEY)
        {
        	/* for EC Public key, we have to look for the curve name, which is stored as an object identifer
        	 *
        	 */
        	if(sslDerd_getNextValue(ps_derdCtx) == SSL_DER_ASN1_OBJECT)
        	{
        		//Store the ID of the curve
        		*p_eccCurve = sslOid_fromDer(ps_derdCtx->s_octVal.pc_data,
                        ps_derdCtx->s_octVal.cwt_len);
        	}
        }
        else
