...
static const uint8_t rac_macPad1[48] = {
...
static const uint8_t rac_macPad2[48] = {
...
//TODO vpy: Add ECDSA
static const uint16_t rai_tlsSHAlgs[]       = {
		((E_SSL_HASH_SHA256 << 8) + (E_SSL_SIGN_RSA & 0xFF)),
		((E_SSL_HASH_MD5 << 8) + (E_SSL_SIGN_RSA & 0xFF)),
		((E_SSL_HASH_SHA1 << 8) + (E_SSL_SIGN_RSA & 0xFF)),
};
...
static const uint8_t rac_cliHello[] =
{ 0x01, 0x00, 0x00, 0x00 /* Client Hello indicator and len (3 bytes), var */
};

static const uint8_t rac_srvHello[] =
{ 0x02, 0x00, 0x00, 0x00 /* Server Hello indicator and len (3 bytes), var */
};

static const uint8_t rac_cert[] =
{ 0x0b, 0x00, 0x00, 0x00 /* Server Certificate indicator and len (3 bytes), var */
};

static const uint8_t rac_srvKeyExch[] =
{ 0x0c, 0x00, 0x00, 0x00 };

static const uint8_t rac_certReq[] =
{ 0x0d, 0x00, 0x00, 0x00, 0x01, 0x01 /* handshake Cert Request indicator and len (3 bytes), var */
};

static const uint8_t rac_srvHelloDone[] =
{ 0x0e, 0x00, 0x00, 0x00 /* server hello done, message length is 0 */
};

static const uint8_t rac_certVerify[] =
{ 0x0F, 0x00, 0x00, 0x00 };

static const uint8_t rac_cliKeyExch[] =
{ 0x10, 0x00, 0x00, 0x00 };

static const uint8_t rac_srvFinish[] =
{ 0x14, 0x00, 0x00 /* Server finished indicator, 1 byte length needs to be added */
};
...
/*===========================================================================*/
static e_sslError_t loc_verifySign(s_sslCtx_t*  ps_sslCtx,
		uint8_t*    pc_tbvParams, size_t sz_inLen,
		uint8_t*    pc_encSign,   size_t sz_ecnSignLen);

static e_sslError_t loc_signHash(s_sslCtx_t* ps_sslCtx,
		uint8_t* pc_in, size_t  sz_inLen,
		uint8_t* pc_out,size_t* sz_outLen);

static const uint8_t* const loc_getFinLabel(s_sslCtx_t *ps_sslCtx, uint8_t isCli);
...
static e_sslError_t loc_prf(s_sslCtx_t* ps_sslCtx,
		uint8_t* pc_secret, size_t  sz_secLen,
		const uint8_t* pc_label,  uint8_t c_lbLen,
		uint8_t* pc_par1,   uint8_t c_par1Len,
		uint8_t* pc_par2,   uint8_t c_par2Len,
		uint8_t* pc_result, size_t  sz_outLen);

static void     loc_prfSSL( uint8_t* pc_sec, size_t cwt_secLen,
		uint8_t* pc_par1,uint8_t cwt_par1Len,
		uint8_t* pc_par2,uint8_t cwt_par2Len,
 ...
 *
 */
static void     loc_prfTLS( e_sslPrf_t  e_prf,
		uint8_t*    pc_secret,       size_t c_secretLen,
		const uint8_t*    pc_label,        uint8_t c_labelLen,
		uint8_t*    pc_seed1,        uint8_t c_seed1Len,
		uint8_t*    pc_seed2,        uint8_t c_seed2Len,
		uint8_t*    pc_out,          size_t sz_outLen);

static uint32_t loc_addPadding(s_sslCtx_t* ps_sslCtx, uint8_t *pc_data,
		uint32_t l_actLen, uint32_t l_blkSize);

static uint32_t loc_rmPadding(s_sslCtx_t* ps_sslCtx,  uint8_t *pc_data, uint32_t l_len,
		uint32_t l_blkSize);

static size_t loc_cpCompositeHs(s_sslCtx_t* ps_sslCtx,
		uint8_t* pc_to, uint8_t* pc_from,
		size_t cwt_len);
/*===========================================================================*/
...
static void loc_setSecParams(s_sslCtx_t*        ps_sslCtx,
...
static e_sslPendAct_t loc_v2UpwardHandler(s_sslCtx_t * ps_sslCtx, uint8_t *pc_rec,
		size_t cwt_recLen);

static e_sslPendAct_t loc_protocolHand(s_sslCtx_t * ps_sslCtx, uint8_t c_event,
		uint8_t *pc_rec, size_t cwt_recLen,
		uint8_t *pc_wData, size_t *cwt_wDataLen);

static e_sslPendAct_t loc_protocolResp(s_sslCtx_t * ps_sslCtx,
		uint8_t *pc_rec, size_t *pcwt_recLen, uint8_t *pInputData,
		size_t uiInputDataLen);

static e_sslPendAct_t loc_smMacEncrypt(s_sslCtx_t * ps_sslCtx,
		uint8_t *pc_rawTxt,size_t cwt_rawTxtLen,
		uint8_t *pc_rec,size_t *pcwt_recLen,
		e_sslRecType_t e_recType);

static int loc_buildRecordHeader(s_sslCtx_t * ps_sslCtx, uint8_t *pc_rec,
		size_t cwt_recLen,uint8_t c_recType);

static uint8_t *loc_matchCipherSpec(s_sslCtx_t* ps_sslCtx, uint8_t *cipherSpecList,
		uint16_t cipherSpecListLen, uint16_t u_bufLen);

static int32_t loc_processExtens(s_sslCtx_t* ps_sslCtx,
		uint8_t* pc_extsStart, uint8_t* pc_hsEnd);
 ...
 */
static int32_t loc_procExtSignAlg(s_sslCtx_t* ps_sslCtx,
		uint8_t* pc_read, uint32_t lLen);
...
static uint8_t loc_getHashSize(e_sslHashAlg_t e_hashAlg)
{
	...
	{
	case E_SSL_HASH_MD5:
		c_hashSize = CW_MD5_DSIZE;
		break;
	case E_SSL_HASH_SHA1:
		c_hashSize = CW_SHA1_DSIZE;
		break;
	case E_SSL_HASH_SHA256:
		c_hashSize = CW_SHA256_DSIZE;
		break;
	default:
...
static uint8_t loc_getHashSizeByPrf(e_sslPrf_t e_prfAlg)
{
	...
	case E_SSL_PRF_MD5_SHA1:
		c_hashSize = CW_MD5_DSIZE + CW_SHA1_DSIZE;
		break;
		...
		uint8_t*    pc_encSign,   size_t sz_ecnSignLen)
{
	...
	uint8_t             ac_sign[CW_MAX_HASHSIZE];
	/* In case of TLS 1.2 we have prepended hash oid */
	size_t              sz_decSignLen = CW_MAX_HASHSIZE + SSL_DER_ASN1_OID_HASH_MAX_LEN;
	...
	if (ps_sslCtx->s_secParams.s_signAlg.c_hash == E_SSL_HASH_INVALID) {
		...
		cw_sha1Ctx_t  cwt_sha1Ctx;
		cw_md5Ctx_t  cwt_md5Ctx;

		cr_digestInit(&cwt_md5Ctx, NULL, 0, E_SSL_HASH_MD5);
		cr_digestUpdate(&cwt_md5Ctx, ps_hsElem->ac_cliRand,
				CLI_RANDSIZE,E_SSL_HASH_MD5);
		cr_digestUpdate(&cwt_md5Ctx, ps_hsElem->ac_srvRand,
				SRV_RANDSIZE, E_SSL_HASH_MD5);
		...
		cr_digestFinish(&cwt_sha1Ctx, &ac_sign[CW_MD5_DSIZE],
				NULL, E_SSL_HASH_SHA1);

		c_hashLen = CW_MD5_SHA1_DSIZE;
	} else {
		/* message digest contexts will be used to verify DH server parameter */
		cw_hashCtx_t  cwt_hashCtx;
	...
	if (sz_ecnSignLen != (size_t)i_signLen + 2) {
		...
		LOG_ERR("Length of signature given in message "
				"not compatible with message length");
		...
		case E_SSL_SIGN_RSA:
			...
			if (cw_rsa_sign_decode(pc_encSign, i_signLen,
					...
					&ps_hsElem->peerPubKey.key.rsaPub) != CW_OK){
			...
			if (ps_sslCtx->e_ver >= E_TLS_1_2) {
				...
				s_sslOctetStr_t s_sigOctStr = {
						.cwt_len = sz_decSignLen,
						.pc_data = ac_decSign,
				};

				if (sslDerd_initDecCtx(&s_derdCtx, &s_sigOctStr) == SSL_DER_ASN1_UNDEF) {
					LOG_ERR("Failed to decode ASN.1 sequence"
					...
					if ((e_derErr != E_SSL_DER_OK) ||
							(sslDerd_getNextValue(&s_derdCtx) != SSL_DER_ASN1_CSEQUENCE)) {
						LOG_ERR("Signature DER ASN.1 should start with Sequence identifier");
						e_derErr = E_SSL_DER_ERR_NO_CSEQUENCE;
					}
		...
		uint8_t* pc_out,size_t* sz_outLen)
{
	...
	if (c_hashType == E_SSL_HASH_INVALID) {
		...
		cr_digestFinish(&cwt_sha1Ctx, &ac_hash[CW_MD5_DSIZE], NULL, E_SSL_HASH_SHA1);
	...
	sz_signLen = (sizeof(ps_sslCtx->ac_socBuf) - ((size_t) pc_in - (size_t) ps_sslCtx->ac_socBuf));
		...
		//case E_SSL_SIGN_
	{
		if (ps_sslCtx->e_ver >= E_TLS_1_2) {
			s_derdCtx_t     	s_derdCtx;
			uint8_t             c_signLen = CW_MAX_HASHSIZE +
		...
		uint8_t* pc_in, size_t sz_inLen)
{
	...
	if (ps_sslCtx->s_secParams.e_prf == E_SSL_PRF_MD5_SHA1) {
		...
		case E_HASHOP_INIT:
			...
			cr_digestInit(&ps_sslCtx->ps_hsElem->u_hashCtx.s_md5Sha1.cwt_md5Ctx,
					NULL, 0, E_SSL_HASH_MD5);
			cr_digestInit(&ps_sslCtx->ps_hsElem->u_hashCtx.s_md5Sha1.cwt_sha1Ctx,
		...
		case E_HASHOP_UPDATE:
			...
			cr_digestUpdate(&ps_sslCtx->ps_hsElem->u_hashCtx.s_md5Sha1.cwt_md5Ctx,
					pc_in, sz_inLen, E_SSL_HASH_MD5);
			cr_digestUpdate(&ps_sslCtx->ps_hsElem->u_hashCtx.s_md5Sha1.cwt_sha1Ctx,
		...
		case E_HASHOP_INIT:
			...
			cr_digestInit(&ps_sslCtx->ps_hsElem->u_hashCtx.cwt_hashCtx,
					NULL, 0, c_hashType);
		...
		case E_HASHOP_UPDATE:
			...
			cr_digestUpdate(&ps_sslCtx->ps_hsElem->u_hashCtx.cwt_hashCtx,
...
static void loc_compHash(s_sslCtx_t* ps_sslCtx,
		const uint8_t* pc_label, uint8_t* pc_res)
{
	e_sslVer_t e_ver = ps_sslCtx->e_ver;
		...
		uint8_t *pc_res)
{
	...
	LOG1_INFO("loc_compHash, param: %s",
			(pc_snd!=NULL)?(char*)pc_snd:"NULL");
	...
	/* ctx->pcwt_md5Ctx */
	CW_MEMCOPY(&cwt_sha1Ctx, &ps_handshElem->u_hashCtx.s_md5Sha1.cwt_sha1Ctx, sizeof(cw_sha1Ctx_t));
	...
	/* Copy of the context */
	CW_MEMCOPY(&cwt_md5Ctx, &ps_handshElem->u_hashCtx.s_md5Sha1.cwt_md5Ctx, sizeof(cw_md5Ctx_t));
		...
		uint8_t*    pc_result)
{
	...
	if (ps_sslCtx->s_secParams.e_prf == E_SSL_PRF_MD5_SHA1){
		cw_md5Ctx_t cwt_md5Ctx;
		cw_sha1Ctx_t cwt_sha1Ctx;
		uint8_t ac_md5hash[CW_MD5_DSIZE];
		uint8_t ac_sha1hash[CW_SHA1_DSIZE];
		...
		CW_MEMCOPY(&cwt_md5Ctx,
				&ps_sslCtx->ps_hsElem->u_hashCtx.s_md5Sha1.cwt_md5Ctx,
				sizeof(cw_md5Ctx_t));

		CW_MEMCOPY(&cwt_sha1Ctx,
				&ps_sslCtx->ps_hsElem->u_hashCtx.s_md5Sha1.cwt_sha1Ctx,
				sizeof(cw_sha1Ctx_t));
		...
		if (pc_label != NULL) {
			loc_prfTLS(E_SSL_PRF_MD5_SHA1,
					...
					ac_md5hash,  CW_MD5_DSIZE,
					ac_sha1hash,  CW_SHA1_DSIZE,
					pc_result, VERIF_HASHSIZE_TLS);
		} else {
			CW_MEMCOPY(pc_result, ac_md5hash, CW_MD5_DSIZE);
			CW_MEMCOPY(pc_result + CW_MD5_DSIZE, ac_sha1hash, CW_SHA1_DSIZE);
		}
	} else {
		cw_hashCtx_t    cwt_hashCtx;
		...
		CW_MEMCOPY(&cwt_hashCtx,
				&ps_sslCtx->ps_hsElem->u_hashCtx.cwt_hashCtx,
				sizeof(cw_hashCtx_t));
		...
		e_sslHashAlg_t  e_hashType)
{
	...
	CW_MEMCOPY(as_temp, pc_seqNum, 8);
	as_temp[8] = c_msgType;
	...
	if (e_hashType == E_SSL_HASH_SHA1)
	{
		...
		cr_digestUpdate(&cw_hashCtx, pc_sec, CW_SHA1_DSIZE, e_hashType);
		...
		cr_digestUpdate(&cw_hashCtx, ac_hashBuf, CW_SHA1_DSIZE, e_hashType);

		cr_digestFinish(&cw_hashCtx, result, NULL, e_hashType);

		cwt_retLen = CW_SHA1_DSIZE;
	}
	else if (e_hashType == E_SSL_HASH_MD5)
	{
		...
		cr_digestUpdate(&cw_hashCtx, pc_sec, CW_MD5_DSIZE, e_hashType);
		...
		cr_digestUpdate(&cw_hashCtx, ac_hashBuf, CW_MD5_DSIZE, e_hashType);

		cr_digestFinish(&cw_hashCtx, result, NULL, e_hashType);

		cwt_retLen = CW_MD5_DSIZE;
	}
		...
		uint8_t* pc_result, size_t  sz_outLen)
{
	...
	switch (ps_sslCtx->e_ver) {
	case E_SSL_3_0:
		loc_prfSSL( pc_secret,  sz_secLen,
				pc_par1,    c_par1Len,
				pc_par2,    c_par2Len,
				pc_result,  sz_outLen);
		break;
		...
		uint8_t* pc_out, size_t cwt_outLen)
{
	...
	uint8_t ac_mix[20];
	uint8_t secBuf[48];
	uint8_t sha1_hash[CW_SHA1_DSIZE];

	/* Two local copies of Hash-Contexts are needed */
	cw_sha1Ctx_t cwt_sha1KbCtx;
	cw_md5Ctx_t cwt_md5KbCtx;
	...
	{
		CW_MEMCOPY(secBuf, pc_sec, cwt_secLen);
		pc_sec = secBuf;
	}
	...
	while (i_bytes > 0)
	{
		...
		/* The inner hash */
		cr_digestInit(&cwt_sha1KbCtx, NULL, 0, E_SSL_HASH_SHA1);
		cr_digestUpdate(&cwt_sha1KbCtx, ac_mix, c_mixCount, E_SSL_HASH_SHA1);
		cr_digestUpdate(&cwt_sha1KbCtx, pc_sec, cwt_secLen, E_SSL_HASH_SHA1);
		cr_digestUpdate(&cwt_sha1KbCtx, pc_par1, cwt_par1Len, E_SSL_HASH_SHA1);
		cr_digestUpdate(&cwt_sha1KbCtx, pc_par2, cwt_par2Len, E_SSL_HASH_SHA1);
		cr_digestFinish(&cwt_sha1KbCtx, sha1_hash, NULL, E_SSL_HASH_SHA1);

		/* The outer hash */
		cr_digestInit(&cwt_md5KbCtx, NULL, 0, E_SSL_HASH_MD5);
		cr_digestUpdate(&cwt_md5KbCtx, pc_sec, cwt_secLen, E_SSL_HASH_MD5);
		cr_digestUpdate(&cwt_md5KbCtx, sha1_hash, CW_SHA1_DSIZE, E_SSL_HASH_MD5);
		cr_digestFinish(&cwt_md5KbCtx, pc_out, NULL, E_SSL_HASH_MD5);
...
static void loc_prfTLS(e_sslPrf_t  e_prf,
		uint8_t*    pc_secret,       size_t sz_secretLen,
		const uint8_t*    pc_label,        uint8_t c_labelLen,
		uint8_t*    pc_seed1,        uint8_t c_seed1Len,
		uint8_t*    pc_seed2,        uint8_t c_seed2Len,
		uint8_t*    pc_out,          size_t sz_outLen)
{
	...
	if (e_prf == E_SSL_PRF_MD5_SHA1) {
		...
		CW_MEMSET(c_prf1Res, 0, c_prfResSize);
		CW_MEMSET(c_prf2Res, 0, c_prfResSize);
		...
		loc_pHash(e_hmacType,pc_secret, sz_secretLen,
				pc_label, c_labelLen,
				pc_seed1, c_seed1Len,
				pc_seed2, c_seed2Len,
				c_prfRes, sz_outLen);
		...
		size_t cwt_len)
{
	...
	case E_SSL_3_0:
		 ...
		 * about this   */
		if ((ps_sslCtx->s_secParams.e_kst != E_SSL_KST_DHE_RSA) &&
				(ps_sslCtx->s_secParams.e_kst != E_SSL_KST_DHE_DSS)) {
	...
	case E_TLS_1_2:
		...
		{
		case E_SSL_KST_ECDHE_ECDSA:
		case E_SSL_KST_ECDHE_RSA:
	...
	if ((cwt_resLen) && (cwt_offset)) {
		CW_MEMMOVE(pc_to, pc_from + cwt_offset, cwt_resLen);
	}
...
static void loc_compKey(s_sslCtx_t * ps_sslCtx, uint8_t b_srvKey)
{
	...
	s_sslHsElem_t*      ps_hsElem;
	s_sslSecParams_t*   ps_secPar;
	cw_rc4Ctx_t*        cwt_rc4Ctx;
	cw_aesCtx_t*        cwt_aesCtx;
	...
	if (b_srvKey == TRUE)
	{
		cwt_rc4Ctx = &(ps_secPar->u_srvKey.cwt_srvRc4Ctx);
		cwt_tdesCtx = &(ps_secPar->u_srvKey.cwt_srv3DesCtx);
		cwt_aesCtx = &(ps_secPar->u_srvKey.cwt_srvAesCtx);
		c_macOff = ps_secPar->c_hmacLen;
		c_keyOff = 2*c_macOff + ps_secPar->c_keyLen;
		c_ivOff = 2*c_macOff + 2*ps_secPar->c_keyLen + ps_secPar->c_blockLen;
		CW_MEMSET(ps_sslCtx->s_sslGut.ac_srvrSeqNum, 0, 8);
		CW_MEMCOPY(ps_secPar->ac_srvSecret,&(ac_keyBlk[c_macOff]), ps_secPar->c_hmacLen);
	} /* if */
	else
	{
		cwt_rc4Ctx = &(ps_secPar->u_cliKey.cwt_cliRc4Ctx);
		cwt_tdesCtx = &(ps_secPar->u_cliKey.cwt_cli3DesCtx);
		cwt_aesCtx = &(ps_secPar->u_cliKey.cwt_cliAesCtx);
		c_macOff = 0;
		...
		CW_MEMCOPY(ps_secPar->ac_cliSecret,&(ac_keyBlk[c_macOff]), ps_secPar->c_hmacLen);
		LOG2_INFO("MAC Key");
	...
	{
		cw_rc4_init(cwt_rc4Ctx, &(ac_keyBlk[c_keyOff]), ps_secPar->c_keyLen);
		LOG2_INFO("Key");
	...
	{
		cw_3des_init(cwt_tdesCtx, /*<--*/
				&(ac_keyBlk[c_keyOff]), ps_secPar->c_keyLen,
				&(ac_keyBlk[c_ivOff]), ps_secPar->c_blockLen, 0);
...
static e_sslPendAct_t loc_selVer(s_sslCtx_t* ps_sslCtx, e_sslVer_t e_ver)
{
			...
			|| (ps_hsElem->e_offerVer > ps_sslCtx->e_ver))
	{
		 ...
		 */
		ret = sslRec_fetchCorrectVersion(ps_sslCtx,
				ps_hsElem->e_offerVer);
	...
	{
		LOG_ERR("%s not allowed, must use %s",sslDiag_getVersion(ps_hsElem->e_offerVer),
		...
		e_sslCipSpec_t     e_cipSpec)
{
	...
	{
	case TLS_RSA_WITH_RC4_128_MD5:
		ps_secPar->e_kst            = E_SSL_KST_RSA;
		ps_secPar->s_signAlg.c_sign = E_SSL_SIGN_RSA;
		ps_secPar->e_cipType        = RC4_STREAM;
		ps_secPar->b_isBlkCip       = FALSE;
		ps_secPar->e_hmacType       = E_SSL_HASH_MD5;
	...
	case TLS_RSA_WITH_3DES_EDE_CBC_SHA:
		ps_secPar->e_kst            = E_SSL_KST_RSA;
		ps_secPar->s_signAlg.c_sign = E_SSL_SIGN_RSA;
		ps_secPar->e_cipType        = TDES_BLOCK;
	...
	case TLS_RSA_WITH_AES_128_CBC_SHA256:
		...
		ps_secPar->b_isBlkCip       = TRUE;
		ps_secPar->c_blockLen       = 16;
		ps_secPar->e_hmacType       = E_SSL_HASH_SHA256;
		ps_secPar->c_keyLen         = 16;
		break;
	case TLS_RSA_WITH_AES_256_CBC_SHA:
		ps_secPar->e_kst            = E_SSL_KST_RSA;
	...
	case TLS_RSA_WITH_AES_256_CBC_SHA256:
		...
		break;
	case TLS_DHE_RSA_WITH_3DES_EDE_CBC_SHA:
		ps_secPar->e_kst            = E_SSL_KST_DHE_RSA;
		ps_secPar->s_signAlg.c_sign = E_SSL_SIGN_RSA;
		ps_secPar->e_cipType        = TDES_BLOCK;
		ps_secPar->b_isBlkCip       = TRUE;
		ps_secPar->c_blockLen       = 8;
	...
	case TLS_ECDHE_ECDSA_WITH_3DES_EDE_CBC_SHA:
		ps_secPar->e_kst            = E_SSL_KST_ECDHE_ECDSA;
		ps_secPar->s_signAlg.c_sign = E_SSL_SIGN_ECDSA;
		ps_secPar->e_cipType        = TDES_BLOCK;
		ps_secPar->b_isBlkCip       = TRUE;
		ps_secPar->c_blockLen       = 8;
		ps_secPar->e_hmacType       = E_SSL_HASH_SHA1;
	...
	case TLS_ECDHE_RSA_WITH_3DES_EDE_CBC_SHA:
		ps_secPar->e_kst            = E_SSL_KST_ECDHE_RSA;
		ps_secPar->s_signAlg.c_sign = E_SSL_SIGN_RSA;
		ps_secPar->e_cipType        = TDES_BLOCK;
		ps_secPar->b_isBlkCip       = TRUE;
		ps_secPar->c_blockLen       = 8;
...
#endif
	default:
		ps_secPar->e_kst            = E_SSL_KST_INVALID;
		ps_secPar->s_signAlg.c_sign = E_SSL_SIGN_INVALID;
		ps_secPar->e_cipType        = INVALID_SYM;
		ps_secPar->b_isBlkCip       = FALSE;
		ps_secPar->c_blockLen       = 0;
		ps_secPar->e_hmacType       = E_SSL_HASH_NONE;
	...
	case E_TLS_1_2:
		...
		if (ps_sslCtx->c_isResumed == TRUE) {
			ps_secPar->s_signAlg.c_sign = ps_sslCtx->ps_hsElem->s_sessElem.s_signAlg.c_sign;
			ps_secPar->s_signAlg.c_hash = ps_sslCtx->ps_hsElem->s_sessElem.s_signAlg.c_hash;
		} else {
			/* By default TLS 1.2 specify to use SHA1 as a hash for all
			 * supported signatures */
			switch (ps_secPar->e_kst) {
			case E_SSL_KST_DHE_RSA:
			case E_SSL_KST_RSA:
			case E_SSL_KST_ECDHE_RSA: //vpy
				ps_secPar->s_signAlg.c_sign = E_SSL_SIGN_RSA;
				ps_secPar->s_signAlg.c_hash = E_SSL_HASH_SHA1;
				break;
			case E_SSL_KST_DHE_DSS:
				ps_secPar->s_signAlg.c_sign = E_SSL_SIGN_DSA;
				ps_secPar->s_signAlg.c_hash = E_SSL_HASH_SHA1;
				break;
			//begin vpy
			case E_SSL_KST_ECDHE_ECDSA:
			...
			default:
				ps_secPar->s_signAlg.c_sign = E_SSL_SIGN_INVALID;
				ps_secPar->s_signAlg.c_hash = E_SSL_HASH_INVALID;
			}
		}
 ...
 */
static uint8_t* loc_matchCipherSpec(s_sslCtx_t* ps_sslCtx, uint8_t *pc_cipSpecList,
		uint16_t i_cipSpecListLen, uint16_t u_bufLen)
{
	...
	*pe_selCip = TLS_NULL_WITH_NULL_NULL;
	...
	for (i = 0; (i < SSL_CIPSPEC_COUNT) && (*pe_selCip == TLS_NULL_WITH_NULL_NULL); i++)
	{
		j = l_start;
		...
		for (; j < i_cipSpecListLen + l_start - 1; j += l_inc) {
			 ...
			 */
			e_sslCipSpec_t cipherSpec =
					(e_sslCipSpec_t)ssl_readInteger(&(pc_cipSpecList[j]), 2);
	...
	if ((pc_ret != NULL) || (l_inc == 3))
	{
		...
		if (ps_sslCtx->e_ver == E_TLS_1_2) {
			ps_sslCtx->s_secParams.s_signAlg.c_hash = E_SSL_HASH_SHA1;
		} else {
			/* For all other versions it's assumed that sha1 + md5 are
			 * supported */
			ps_sslCtx->s_secParams.s_signAlg.c_hash = E_SSL_HASH_INVALID;
		}
	}
	/*
		...
		uint8_t* pc_hsEnd)
{
	...
	{
		LOG_ERR("Extensions are longer (%zu) than remaining "
				"handshake length (%i)",l_extsLen, (pc_hsEnd - pc_extsStart));
		c_ret = -1;
	} /* if */
	...
	while ((pc_curExt < pc_hsEnd) && (c_ret >= 0))
	{
		...
		case TLS_EXTENSION_RENEGOTIATION_INFO:
		{
			...
			/* So this is a re-negotiation */
						...
						(b_isCli && (*pc_curExt != (l_lvdLen * 2))))
				{
					 ...
					 */
					if (CW_MEMCMP(pc_curExt + 1, ps_g->ac_cliVerifyData, l_lvdLen) != 0)
					{
						/*
					...
					else if (ps_sslCtx->b_isCli == TRUE)
					{
						 ...
						 */
						if (CW_MEMCMP(pc_curExt + 1 + l_lvdLen,
								ps_g->ac_srvVerifyData,
								l_lvdLen) != 0)
						{
							/*
		...
		case TLS_EXTENSION_SIGNATURE_ALGORITHMS: {
			...
			if (ps_sslCtx->e_ver > E_TLS_1_1) {
				...
				if (i_tmpLen == (l_curExtLen - 2)) {
					...
					l_curExtLen-=2;
					/* Parse and process signature_algorithms extension */
					c_ret = loc_procExtSignAlg(ps_sslCtx, pc_curExt, l_curExtLen);
				} else {
					/*
					 * Length was not OK
					 */
		...
		case TLS_EXTENSION_ELLIPTIC_CURVES: //vpy
			...
			if (ps_sslCtx->e_ver == E_TLS_1_2)
			{
				...
				int16_t signKeyCurve;
				int16_t signKeyCurveOk = 0;
				...
				//get curve used for certificate if ECDSA key/certificate
				if(ps_sslCtx->ps_sslSett->p_ECCMyPrivKey!=NULL)
				{
					signKeyCurve = cw_ecc_convert_curveName_curveNumer(ps_sslCtx->ps_sslSett->p_ECCMyPrivKey->dp->name);
					signKeyCurveOk = 0;
				}

				//get number of curves proposed by client
				i_tmpLen = ssl_readInteger(pc_curExt,2)/2; //one curve is 2 bytes, so number of curves is the half of the length
				pc_curExt+=2;
				l_curExtLen-=2;

				//loop for each curve proposed by client
				int i;
				for(i=0; i<i_tmpLen; i++)
				{
					//read curve i proposed  by client
					...
					uint8_t j;
					for(j=0; (j<numberOfCurves && ps_sslCtx->s_secParams.eccChoosenCurve==0xFFFF); j++)
					{
					...
					//compare curve proposed by client and curve used in our certificate, if ECDSA key/certificate
					if(ps_sslCtx->ps_sslSett->p_ECCMyPrivKey!=NULL)
					{
						if(signKeyCurve == i_tmpCurve)
						{
							signKeyCurveOk = 1; //Correspondence found. The client is able to handle the curve
						}
					}
				}
				if((ps_sslCtx->ps_sslSett->p_ECCMyPrivKey!=NULL) && (signKeyCurveOk!=1))
				{
					LOG1_INFO("Our EC public key, provided by certificate is on the curve: %d - %s"
							"is not compatible with curves supported by client. handshake failure",
							cw_ecc_convert_curveName_curveNumer(ps_sslCtx->ps_sslSett->p_ECCMyPrivKey->dp->name),
							ps_sslCtx->ps_sslSett->p_ECCMyPrivKey->dp->name);

					ps_sslCtx->e_lastError = E_SSL_ERROR_PROTO;
					ps_g->e_alertType  = E_SSL_ALERT_HANDSH_FAIL;
					ps_g->e_smState    = E_SSL_SM_SEND_FATAL_ALERT;
					break;
				}
	...
	while (l_len >= 2) {
		...
		if (ps_sslCtx->b_isCli) {
			for (i = 0; i < SSL_MAX_EXTS; i++) {
				...
				if (ps_sslCtx->ps_hsElem->pe_reqExts[i] == TLS_EXTENSION_SIGNATURE_ALGORITHMS) {
			...
			//TODO vpy ECDSA: Hash should be chosen in a list of the hash we support
			if ((ps_sslCtx->ps_sslSett->s_certSignHashAlg.c_sign == c_curSign) &&
					(ps_sslCtx->ps_sslSett->s_certSignHashAlg.c_hash == c_curHash) &&
					(c_curSign != E_SSL_SIGN_ANONY)) {
				ps_sslCtx->s_secParams.s_signAlg.c_hash = c_curHash;
				ps_sslCtx->s_secParams.s_signAlg.c_sign = c_curSign;
...
static uint8_t* loc_appendExtens(s_sslCtx_t* ps_sslCtx, uint8_t* pc_in)
{
	...
	if(ps_sslCtx->b_isCli) //is client ?
	{
		...
		if ((ps_sslCtx->c_isRenegOn == TRUE) && (ps_sslCtx->c_secReneg == TRUE))
		{
			...
			else
			{
				 ...
				 */
				CW_MEMCOPY(pc_out, &ps_sslCtx->s_sslGut.ac_cliVerifyData[0],l_outLen);
				pc_out += l_outLen;
				/*
				...
				if (ps_sslCtx->b_isCli == FALSE)
				{
					 ...
					 */
					CW_MEMCOPY(pc_out, &ps_sslCtx->s_sslGut.ac_srvVerifyData[0], l_outLen);
					pc_out += l_outLen;
					/*
				...
				(ps_sslCtx->ps_sslSett->s_certSignHashAlg.c_hash != E_SSL_HASH_INVALID))
		{
			...
			for (i = 0; i < (sizeof(rai_tlsSHAlgs)/sizeof(rai_tlsSHAlgs[0])); i++) {
		...
		if ((ps_sslCtx->e_ver == E_TLS_1_2))
		{
			...
			for(i=0; i<SSL_CIPSPEC_COUNT; i++)
			{
				if(
						...
						(ps_sslCtx->s_sslGut.ae_cipSpecs[i] == TLS_ECDHE_RSA_WITH_3DES_EDE_CBC_SHA) ||
						(ps_sslCtx->s_sslGut.ae_cipSpecs[i] == TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA) ||
						(ps_sslCtx->s_sslGut.ae_cipSpecs[i] == TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA)
				)
				{
					isECCPresent = 1;
				}
			...
			if(isECCPresent==1)
			{
				...
				uint8_t numberOfCurves = cw_ecc_getSupportedCurves(supportedCurves);
				uint8_t i;
	...
	else //is server
	{
		...
		if ((ps_sslCtx->e_ver == E_TLS_1_2) &&
				(
						(ps_sslCtx->s_sslGut.e_pendCipSpec == TLS_ECDHE_ECDSA_WITH_3DES_EDE_CBC_SHA) ||
						(ps_sslCtx->s_sslGut.e_pendCipSpec == TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA) ||
						(ps_sslCtx->s_sslGut.e_pendCipSpec == TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA) ||
						(ps_sslCtx->s_sslGut.e_pendCipSpec == TLS_ECDHE_RSA_WITH_3DES_EDE_CBC_SHA) ||
						(ps_sslCtx->s_sslGut.e_pendCipSpec == TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA) ||
						(ps_sslCtx->s_sslGut.e_pendCipSpec == TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA)
				)
		)
		{
			/*
...
void ssl_destroyKeys(s_sslCtx_t * ps_sslCtx)
{
	...
	km_dhe_releaseKey();
	ps_sslCtx->s_secParams.pcwt_dheKey = NULL;
...
static e_sslPendAct_t loc_protocolResp(s_sslCtx_t * ps_sslCtx,
		uint8_t *pc_rec,  size_t *pcwt_recLen,
		uint8_t *pc_inData,  size_t cwt_inDataLen)
{
	...
	case E_SSL_SM_SEND_FATAL_ALERT:
		...
		LOG1_INFO("%p| Sending %s %s Alert", ps_sslCtx,
				(ps_sslGut->e_smState == E_SSL_SM_SEND_FATAL_ALERT)?"fatal":"warning",
		...
		case E_SSL_SM_SEND_CLIENT_HELLO:
			...
			case E_SSL_ASM_STEP1:
				LOG1_INFO("%p| SEND_CLIENT_HELLO ASM_STEP1 (ClientHello)",
						ps_sslCtx);
				CW_MEMCOPY(pc_rec, (void*)rac_cliHello,
						sizeof(rac_cliHello));
				...
				*pc_write++ = SSL_VERSION_GET_MIN(ps_sslCtx->e_ver);
				...
				CW_MEMCOPY(pc_write,ps_hsElem->ac_cliRand,CLI_RANDSIZE);
				...
				if (ps_sslCtx->c_isResumed == TRUE)
				{
					...
					*pc_write++ = SESSID_SIZE;
					/* write Session ID */
					CW_MEMCOPY(pc_write, ps_hsElem->s_sessElem.ac_id,
							SESSID_SIZE);
					pc_write += SESSID_SIZE;
				} else {
				...
				*pc_write++ = 0x00; /* No compression */
				...
				(void) *ssl_writeInteger(pc_rec + 1,
						cwt_hashLen - HS_HEADERLEN, 3);
				ps_sslGut->e_recordType = E_SSL_RT_HANDSHAKE;
				ps_sslGut->e_asmCtrl = E_SSL_ASM_FINISH;
			...
			case E_SSL_SM_SEND_SERVER_HELLO:
				...
				case E_SSL_ASM_START: /* Prepare Server Hello */
					...
					CW_MEMCOPY(pc_rec, (void*)rac_srvHello,
							sizeof(rac_srvHello));
					...
					if (ps_sslCtx->e_ver == E_VER_DCARE)
					{
						 ...
						 */
						LOG_ERR("%p| Protocol version not set correctly, "
								"stop handshake here!",  ps_sslCtx);
						ps_sslCtx->e_ver = SSL_DEFAULT_SSL_TLS_VERSION;
					...
					*pc_write++ = SSL_VERSION_GET_MIN(ps_sslCtx->e_ver);
					...
					CW_MEMCOPY(pc_write, ps_hsElem->ac_srvRand, SRV_RANDSIZE);
					pc_write += SRV_RANDSIZE;

					/* Insert length of the s_desc */
					*pc_write++ = SESSID_SIZE;
					CW_MEMCOPY(pc_write, ps_hsElem->s_sessElem.ac_id, SESSID_SIZE);
				...
				case E_SSL_ASM_STEP1:
				{
					...
					LOG1_INFO("%p| SEND_SERVER_HELLO ASM_STEP1 (Certificate)",
							ps_sslCtx);
					CW_MEMCOPY(pc_rec, (void*)rac_cert,
							sizeof(rac_cert));
					...
					case E_SSL_KST_RSA:
						...
						if ((ps_sslCtx->e_authLvl & ~E_SSL_MUST_VERF_SRVCERT) != E_SSL_NO_AUTH) {
					...
					case E_SSL_KST_DHE_DSS:
					case E_SSL_KST_ECDHE_RSA: //vpy
					case E_SSL_KST_ECDHE_ECDSA: //vpy
						ps_sslGut->e_asmCtrl = E_SSL_ASM_STEP2;
						break;
					default:
						LOG_INFO("%p| Key share type is not supported",
								ps_sslCtx);
						ps_sslGut->e_asmCtrl = E_SSL_ASM_START;
						ps_sslGut->e_alertType = E_SSL_ALERT_HANDSH_FAIL;
						ps_sslGut->e_smState = E_SSL_SM_SEND_FATAL_ALERT;
				...
				case E_SSL_ASM_STEP2:
				{
					...
					/* copy record header */
					CW_MEMCOPY(pc_rec, rac_srvKeyExch, sizeof(rac_srvKeyExch));
					...
					case E_SSL_KST_ECDHE_ECDSA:
						...
						if(cw_ecc_makeKey(&(ps_secPar->eccKey), ps_secPar->eccChoosenCurve)!=CRYPT_OK)
						{
							LOG_INFO("%p| Couldn't create new ECHE key", ps_sslCtx);
							return (E_PENDACT_COM_CIPHER_CLOSE);
						}
						ps_secPar->c_useEccKey = TRUE; //store the fact we used a ECC key, to be able to free it at the end!
						...
						if(cw_ecc_export_public((pc_write+4), &cwt_exportLen, &(ps_secPar->eccKey))!=CRYPT_OK)
						{
					...
					default:
						 ...
						 * forward secrecy */
						if ((ps_secPar->pcwt_dheKey = km_dhe_getKey()) == NULL) {
						...
						cw_dhe_export_pgY(pc_write, &cwt_exportLen,
								ps_secPar->pcwt_dheKey, &ps_hsElem->pcwt_dheP);
					...
					(void) ssl_writeInteger(pc_rec + 1,
							cwt_exportLen + cwt_hashLen, 3);

					/* Final length is calculated as follows
					 * Length of a pgy part, length of a signed hash and length
					 * of a header */
					cwt_hashLen = *pcwt_recLen = cwt_exportLen + cwt_hashLen + sizeof(rac_srvKeyExch);
				...
				case E_SSL_ASM_STEP3:
				{
					...
					{
						LOG1_ERR("%p| CertRequest should be constructed, "
								"but list is not initialised!", ps_sslCtx);
						if ((ps_sslCtx->e_authLvl & E_SSL_MUST_AUTH) == E_SSL_MUST_AUTH)
						{
							LOG_ERR("%p| Client Authentication is mandatory, "
									"so cancel the handshake here",ps_sslCtx);
						...
						{
							LOG1_INFO("%p| Client Authentication is not "
									"mandatory, so fall through to send "
									"ServerHelloDone", ps_sslCtx);
							ps_sslGut->e_asmCtrl = E_SSL_ASM_STEP4;
							/*
					...
					else
					{
						...
						CW_MEMCOPY(pc_rec, (void*)rac_certReq,sizeof(rac_certReq));
						...
						sslConf_getCertReqList(ps_gSett->ps_caCertsListHead,
								pc_write, &listLen);
						cwt_hashLen += listLen;
						pc_write += listLen;
				...
				case E_SSL_ASM_STEP4:
					LOG1_INFO("%p| SEND_SERVER_HELLO ASM_STEP4 (SrvHelloDone)",
							ps_sslCtx);
					CW_MEMCOPY(pc_rec, (void*)rac_srvHelloDone,
							sizeof(rac_srvHelloDone));
				...
				case E_SSL_SM_SEND_CLIENT_FINISH:
					...
					case E_SSL_ASM_START:
						...
						{
							LOG1_INFO("%p| SEND_CLIENT_FINISH ASM_START (Cert)",
									ps_sslCtx);
							CW_MEMCOPY(pc_rec, (void*)rac_cert, sizeof(rac_cert));
							pc_write = pc_rec + sizeof(rac_cert);
					...
					case E_SSL_ASM_STEP1:
						LOG1_INFO("%p| SEND_CLIENT_FINISH ASM_STEP1 (CliKeyExchange)",ps_sslCtx);
						CW_MEMCOPY(pc_rec, (void*)rac_cliKeyExch, sizeof(rac_cliKeyExch));
						...
						case E_SSL_KST_RSA:
							...
							if (ps_sslCtx->e_ver == E_VER_DCARE)
							{
								LOG_ERR("%p| This shouldn't happen here, no version set!", ps_sslCtx);
								return (E_PENDACT_COM_CIPHER_CLOSE);
							}
							...
							CW_MEMCOPY(ps_hsElem->s_sessElem.ac_msSec, pc_write, PREMSSEC_SIZE);
							...
							ssl_writeInteger(pc_rec + 1, cwt_hashLen + IFSSL30_LENOFF(ps_sslCtx->e_ver), 3);
							ssl_writeInteger(pc_write, cwt_hashLen, IFSSL30_LENOFF(ps_sslCtx->e_ver));
						...
						case E_SSL_KST_DHE_RSA:
							...
							if (cw_dhe_sharedSec_with_p(
									&ps_hsElem->cwt_dheCliPrivKey,
									&ps_hsElem->cwt_dheSrvPubKey,
									&ps_hsElem->pcwt_dheP, pc_write,
									&cwt_hashLen) != CW_OK)
							{
							...
							if (cw_dhe_export_Y(pc_write, &cwt_hashLen,
									&ps_hsElem->cwt_dheCliPrivKey) != CW_OK)
							{
						...
						case E_SSL_KST_ECDHE_ECDSA:
							...
							if(cw_ecc_sharedSecret(&(ps_secPar->eccKey), &(ps_hsElem->peerPubKey.key.eccPub), pc_write, &cwt_hashLen)!=CRYPT_OK)
							{
						...
						default:
							LOG_ERR("%p| Handshake Type 0x%X can't be handled",
									ps_sslCtx,ps_secPar->e_kst);
							ps_sslCtx->e_lastError = E_SSL_ERROR_CRYPTO;
							return (E_PENDACT_COM_CIPHER_CLOSE);
							break;
						}
						...
						case E_SSL_ASM_STEP2:
									...
									&& (ps_sslCtx->ps_lastCliAuthCertChain != NULL))
							{
								...
								LOG1_INFO("%p| SEND_CLIENT_FINISH ASM_STEP2 (CertVerify)", ps_sslCtx);
								CW_MEMCOPY(pc_rec, (void*)rac_certVerify, sizeof(rac_certVerify));
                        ...
                        e_result = cw_rsa_sign_encode(ac_hash, c_hashLen,
								...
								if (cw_rsa_sign_encode(pc_write + 2, c_hashLen, pc_write + 2,
										&cwt_hashLen,
										ps_sslCtx->ps_sslSett->pcwt_rsaMyPrivKey) != CW_OK)
								{
						...
						case E_SSL_ASM_STEP3:
							LOG1_INFO("%p| SEND_CLIENT_FINISH ASM_STEP3 (ChangeCipherSpec)",ps_sslCtx);
						...
						case E_SSL_ASM_STEP4:
							...
							CW_MEMCOPY(pc_write, (void*)rac_srvFinish, sizeof(rac_srvFinish));
							...
							*pc_write++ = ps_sslGut->c_verifyDataLen;
							...
							CW_MEMCOPY(ps_sslGut->ac_cliVerifyData, pc_write, ps_sslGut->c_verifyDataLen);
							pc_write += ps_sslGut->c_verifyDataLen;
							cwt_hashLen = *pcwt_recLen = pc_write - pc_rec;
					...
					case E_SSL_SM_SEND_SERVER_FINISH:
						...
						case E_SSL_ASM_STEP1:
							...
							*pc_write++ = ps_sslCtx->s_sslGut.c_verifyDataLen;
							...
							CW_MEMCOPY(ps_sslCtx->s_sslGut.ac_srvVerifyData, pc_write, ps_sslCtx->s_sslGut.c_verifyDataLen);
							pc_write += ps_sslCtx->s_sslGut.c_verifyDataLen;
							cwt_hashLen = *pcwt_recLen = pc_write - pc_rec;
						...
						case E_SSL_SM_SEND_SERVER_HELLO_FINISH:
							...
							case E_SSL_ASM_START: /* Prepare Server Hello */
								...
								CW_MEMCOPY(pc_rec, (void*)rac_srvHello, sizeof(rac_srvHello));
								pc_write = pc_rec + sizeof(rac_srvHello);
								/* sj check behaviour - should be fine, session resumption works well */
								if (ps_sslCtx->e_ver == E_VER_DCARE)
								{
									LOG_INFO("%p| Protocol version not set correctly, use "
											"last used version out of session cache", ps_sslCtx);
									ps_sslCtx->e_ver = ps_hsElem->s_sessElem.e_lastUsedVer;
								}
								*pc_write++ = SSL_VERSION_GET_MAJ(ps_sslCtx->e_ver);
								*pc_write++ = SSL_VERSION_GET_MIN(ps_sslCtx->e_ver);
							...
							case E_SSL_ASM_STEP2:
								...
								*pc_write++ = ps_sslGut->c_verifyDataLen;
								...
								CW_MEMCOPY(ps_sslGut->ac_srvVerifyData, pc_write, ps_sslGut->c_verifyDataLen);
								pc_write += ps_sslGut->c_verifyDataLen;
								cwt_hashLen = *pcwt_recLen = pc_write - pc_rec;
	...
	loc_hash(E_HASHOP_UPDATE, ps_sslCtx,
			pc_rec + l_blockLen, cwt_hashLen - l_blockLen);

	LOG2_HEX(pc_rec, *pcwt_recLen);

	DbgPrintDigestStates(DBG_LOW, &ps_hsElem->cwt_sha1Ctx, &ps_hsElem->cwt_md5Ctx);
		...
		size_t cwt_recLen)
{
	...
	CW_MEMCOPY(ps_hsElem->ac_cliRand + 32 - cwt_cliChallengeLen,
		...
		uint8_t *pc_wData, size_t *cwt_wDataLen)
{
	...
	case E_PENDACT_CLI_SRVCERTCHAIN:
	{
		...
		if (*p_result != E_SSL_VERIFRES_SUCCESS)
		{
			...
			{
				LOG_WARN("%p| Certificate can't be verified, but we continue "
						"since verification of the server certificate "
						"is not mandatory",ps_sslCtx);
			}
		} /* if */
	...
	if ((ps_sslCtx->b_isCli == FALSE) && (ps_sslGut->b_isComposite == TRUE))
	{
		...
		if (ps_sslGut->e_smState == E_SSL_SM_WAIT_CLIENT_KEYEXCHANGE)
		{
			...
			if (*pc_hsBuff == CLIENT_KEY_EXCHANGE)
			{
				...
				LOG1_INFO("%p| Received CLIENT_KEY_EXCHANGE (composite)", ps_sslCtx);
			...
			{
				LOG1_ERR("%p| CLIENT_KEY_EXCHANGE (composite) expected "
						"but not received", ps_sslCtx);
				LOG2_HEX(pc_hsBuff, cwt_hsBuffLen);
				return (E_PENDACT_GEN_WAIT_EVENT);
			} /* else */
		}
		else if (ps_sslGut->e_smState == E_SSL_SM_WAIT_CLIENT_CERT_VERIFY)
		{
			...
			if (cwt_hsBuffLen + REC_HEADERLEN >= ps_hsElem->cwt_hsBufLen)
			{
				ps_sslGut->b_isComposite = FALSE;
			...
			if (*pc_hsBuff == CERTIFICATE_VERIFY)
			{
				...
				CW_MEMCOPY(pc_wData+VERIF_HASHSIZE, pc_hsBuff+6, cwt_len);
			...
			{
				LOG1_INFO("%p| CertVerify(composite) expected but not received", ps_sslCtx);
				LOG2_HEX(pc_hsBuff, cwt_hsBuffLen);
				return (E_PENDACT_GEN_WAIT_EVENT);
			} /* else */
		} /* else if */
	...
	case E_SSL_RT_ALERT:
	{
		...
		if (pc_rec[6 + l_blkLen] == E_SSL_ALERT_CLOSE_NOTIFY)
		{
			...
			ps_hsElem->s_sessElem.s_signAlg.c_sign = ps_secPar->s_signAlg.c_sign;
			ps_hsElem->s_sessElem.s_signAlg.c_hash = ps_secPar->s_signAlg.c_hash;
			ps_sslGut->e_smState = E_SSL_SM_SHUTDOWN_COMPLETE;
#endif
			break;
		}
	...
	case E_SSL_RT_HANDSHAKE:
	{
		...
		do
		{
			...
			if ((ps_sslCtx->e_ver > E_TLS_1_0) && (ps_secPar->b_isBlkCip == TRUE)
			...
			case CLIENT_HELLO:
			{
				...
				if (ps_sslGut->e_smState != E_SSL_SM_WAIT_CLIENT_HELLO)
				{
					...
					{
						LOG_ERR("%p| E_PENDACT_PROTOERR, state should "
								"be WAIT_CLIENT_HELLO but state is %s",
								ps_sslCtx, sslDiag_getSMState(ps_sslGut->e_smState));
					...
					{
						LOG_INFO("%p| Client tries to renegotiate, but "
								"renegotiation has been disabled",ps_sslCtx);
						ps_sslCtx->e_lastError     = E_SSL_ERROR_PROTO;
				 ...
				 */
				CW_MEMCOPY(ps_hsElem->ac_cliRand, pc_hsBuff + 6, CLI_RANDSIZE);
				...
				pc_tmpRead = pc_tmpRead + 2 + 256*(*pc_tmpRead) + *(pc_tmpRead+1);
				...
				if (cwt_len > SESSID_SIZE) {
					LOG_ERR("Length of Session ID in ClientHello (%u "
							"bytes) larger than maximum (%u bytes)",
							cwt_len, SESSID_SIZE);
				...
				if (cwt_len > 0)
				{
					CW_MEMCOPY(ps_hsElem->s_sessElem.ac_id, pc_hsBuff+39, cwt_len);
			...
			case SERVER_HELLO:
				...
				{
					LOG_ERR("%p| E_PENDACT_PROTOERR, state should be "
							"WAIT_SERVER_HELLO but state is %s",
							ps_sslCtx, sslDiag_getSMState(ps_sslGut->e_smState));
				...
				case E_TLS_1_2:
					...
					break;
				default:
					LOG_ERR("%p| Version is not known, can't set "
							"c_verifyDataLen", ps_sslCtx);
					break;
				} /* switch */

				pc_hsBuff += 2;

				CW_MEMCOPY(ps_hsElem->ac_srvRand, pc_hsBuff, SRV_RANDSIZE);
				pc_hsBuff += SRV_RANDSIZE; /* We jump over ServerRandom(32B) */
				cwt_len = *pc_hsBuff++; /* We read ID len */
				if (cwt_len == 0)
				{
					ps_sslCtx->c_isResumed = FALSE;
					LOG1_INFO("%p| No Session ID received! "
							"Resumption disabled.", ps_sslCtx);
				}
				else
				{
					...
					if (cwt_len > SESSID_SIZE)
					{
						LOG_ERR("Length of Session ID in ServerHello (%u "
								"bytes) larger than maximum (%u bytes)",
								cwt_len, SESSID_SIZE);
						/* send an illegal_parameter alert */
						ps_sslCtx->e_lastError = E_SSL_ERROR_PROTO;
					...
					if (ps_sslCtx->c_isResumed == TRUE)
					{
						if (CW_MEMCMP(ps_hsElem->s_sessElem.ac_id,
								pc_hsBuff, cwt_len) != 0)
						{
							LOG1_INFO("%p| Session ID received but not "
									"equal! Resumption disabled.", ps_sslCtx);
							LOG2_RAW("\nShould be:");
							LOG2_HEX(ps_hsElem->s_sessElem.ac_id, cwt_len);
							LOG2_RAW("is:");
							LOG2_HEX(pc_hsBuff, cwt_len);
							ps_sslCtx->c_isResumed = FALSE;
							CW_MEMCOPY(ps_hsElem->s_sessElem.ac_id, pc_hsBuff, cwt_len);
							ps_hsElem->s_sessElem.s_desc = sslSesCache_getNewSessId(ps_sslCtx->ps_sslSett->ps_sessCache);
						} /* if */
						else
						{
							LOG1_INFO("%p| Session ID received! "
									"Resumption enabled.", ps_sslCtx);
						} /* else */
					} /* if */
					else
					{
				 ...
				 */
				ps_sslGut->e_pendCipSpec =
						(e_sslCipSpec_t)ssl_readInteger(pc_hsBuff, 2);
				...
				LOG_HEX_NAME("ServerHello extensions", pc_hsBuff, pc_hsBuffEnd - pc_hsBuff);
				...
				case TLS_EXTENSION_EC_POINT_FORMATS:
					...
					if(isOk==0)
					{
						LOG_ERR("server doesn't support uncompressed point format");
				...
				case CERTIFICATE:
					...
					if (ps_sslGut->e_smState == E_SSL_SM_WAIT_CLIENT_CERTIFICATE)
					{
						...
						LOG1_INFO("%p| Received CERTIFICATE(Client Authentication)",ps_sslCtx);
						LOG2_HEX(pc_hsBuff, cwt_hsBuffLen);
						...
						if (cwt_recLen - cwt_hsBuffLen > REC_HEADERLEN)
						{
							...
							LOG1_INFO("%p| performing Composite Handling", ps_sslCtx);
							/* Save the 2 other handshakes for later use */
							if (cwt_recLen - cwt_hsBuffLen < ps_hsElem->cwt_hsBufLen)
							{
								CW_MEMCOPY(ps_hsElem->ac_hsBuf,
										pc_hsBuff + cwt_hsBuffLen,
										cwt_recLen - cwt_hsBuffLen);
								ps_hsElem->cwt_hsBufLen = cwt_recLen - cwt_hsBuffLen;
							}
							else
							{
								LOG_ERR("%p| E_PENDACT_PROTOERR", ps_sslCtx);
						...
						if (ssl_readInteger(pc_hsBuff + HS_HEADERLEN, 3) > 0)
						{
							...
							*cwt_wDataLen = cwt_len;
							...
							sslCert_decodeInit(&ps_sslGut->s_peerCertInfo,&ClientCert);

							sslCert_decodeCert(&ps_sslGut->s_peerCertInfo);

							sslCert_decodeTbsCert(&ps_sslGut->s_peerCertInfo);

							if (sslConf_certHook(ps_sslCtx,&ps_sslGut->s_peerCertInfo) != TRUE)
							{
								/* Customer did not accept the certificate */
								LOG_ERR("%p| Certificate was not accepted", ps_sslCtx);
								ps_sslGut->e_alertType = E_SSL_ALERT_BAD_CERT;
								ps_sslGut->e_smState = E_SSL_SM_SEND_FATAL_ALERT;
								return E_PENDACT_SCACHE_RM;
							}
						...
						else
						{
							...
							if ((ps_sslCtx->e_authLvl & E_SSL_MUST_AUTH) == E_SSL_MUST_AUTH) {
								...
								/* Customer did not accept the certificate */
								LOG_ERR("%p| Empty Certificate message was not accepted"
										" according to authentication behavior E_SSL_MUST_AUTH", ps_sslCtx);
								ps_sslGut->e_alertType = E_SSL_ALERT_HANDSH_FAIL;
								ps_sslGut->e_smState = E_SSL_SM_SEND_FATAL_ALERT;
								return E_PENDACT_SCACHE_RM;
							} else {
					...
					else if (ps_sslGut->e_smState == E_SSL_SM_WAIT_CERT)
					{
						...
						LOG_INFO("ac_hsBuf pointer = %p and hsBufLen = %zu "
								"local hsBuf = %p",ps_hsElem->ac_hsBuf,
								ps_hsElem->cwt_hsBufLen,pc_hsBuff);
						LOG2_HEX(pc_hsBuff, cwt_hsBuffLen);
						cwt_len = cwt_hsBuffLen - HS_HEADERLEN;
						...
						*cwt_wDataLen = cwt_len;
						...
						if (ps_sslCtx->s_secParams.e_kst == E_SSL_KST_RSA)
						{
							if ((ps_sslCtx->e_authLvl & E_SSL_MUST_AUTH) == E_SSL_MUST_AUTH)
				...
				case SERVER_KEY_EXCHANGE:
				{
					...
					case E_SSL_KST_ECDHE_RSA:  //vpy
						...
						if(*pc_hsBuff!=0x3)
						{
							LOG_ERR("Received unexpected curve type: %zu", *pc_hsBuff);
						...
						//read/import pubkey
						if(cw_ecc_import_public(pc_hsBuff, cwt_len, &(ps_hsElem->peerPubKey.key.eccPub)) != CRYPT_OK)
						{
						...
						ps_hsElem->peerPubKey.type = KEY_ECC_PUB;
					...
					default:
						...
						if (loc_verifySign(ps_sslCtx,
								pc_pqy, cwt_len,
								pc_hsBuff, cwt_hsBuffLen - HS_HEADERLEN - cwt_len)
								!= E_SSL_NO_ERROR)
						{
							LOG_ERR("%p| E_PENDACT_PROTOERR, state is %s", ps_sslCtx,
									sslDiag_getSMState(ps_sslGut->e_smState));
							...
							LOG_INFO("Verification of signature of "
									"server DH parameters succeeded");
						}
						break;
					}
				...
				case CERTIFICATE_REQUEST:
					...
					{
						LOG_ERR("%p| E_PENDACT_PROTOERR, state should be "
								"WAIT_SERVER_HELLO_DONE or WAIT_CERT_REQUEST "
								"but state is %s", ps_sslCtx,
								sslDiag_getSMState(ps_sslGut->e_smState));
					...
					if (ps_sslCtx->ps_sslSett->ps_certChainListHead == NULL)
					{
						if ((ps_sslCtx->e_authLvl & E_SSL_MUST_AUTH)
								== E_SSL_MUST_AUTH)
						{
							LOG_INFO("%p| Received Certificate Request "
									"and we must authenticate, but no "
									"Client Certificate available",ps_sslCtx);
							return (E_PENDACT_PROTOERR);
						}
						else
						{
							LOG1_INFO("%p| Received Certificate Request "
									"and no Client Certificate available, "
									"but we must not authenticate",ps_sslCtx);
					...
					else
					{
						cwt_len = cwt_hsBuffLen - HS_HEADERLEN;
						pc_hsBuff += HS_HEADERLEN;
						ps_sslCtx->ps_lastCliAuthCertChain =
								sslConf_cmpCertReqList(ps_sslCtx,
										ps_sslCtx->ps_sslSett->ps_certChainListHead,
										pc_hsBuff, cwt_len);
						/*
						...
						if (ps_sslCtx->ps_lastCliAuthCertChain == NULL)
						{
							...
							{
								LOG_INFO("%p| Received Certificate Request "
										"and we must authenticate, but our "
										"certificate can't be handled "
										"by the server", ps_sslCtx);
								return (E_PENDACT_PROTOERR);
							}
							else
							{
								LOG1_INFO("%p| Received Certificate Request "
										"and our certificate can't be "
										"handled by the server, but we "
										"must not authenticate",ps_sslCtx);
							}
						}
					}
					ps_sslGut->e_smState = E_SSL_SM_WAIT_SERVER_HELLO_DONE;
				...
				case SERVER_HELLO_DONE:
					...
					{
						LOG_ERR("%p| E_PENDACT_PROTOERR, state should be "
								"WAIT_SERVER_HELLO_DONE but state is %s",
								ps_sslCtx, sslDiag_getSMState(ps_sslGut->e_smState));
				...
				case CLIENT_KEY_EXCHANGE:
				{
					...
					case E_SSL_MUST_VERF_SRVCERT_AND_E_SSL_SHOULD_AUTH:
						...
						{
							LOG_ERR("%p| E_PENDACT_PROTOERR, state "
									"should be WAIT_CLIENT_KEYEXCHANGE/CERTIFICATE "
									"but state is %s", ps_sslCtx,
									sslDiag_getSMState(ps_sslGut->e_smState));
					...
					case E_SSL_MUST_VERF_SRVCERT_AND_E_SSL_MUST_AUTH:
						...
						{
							LOG_ERR("%p| E_PENDACT_PROTOERR, state "
									"should be E_SSL_SM_WAIT_CLIENT_KEYEXCHANGE "
									"but state is %s", ps_sslCtx,
									sslDiag_getSMState(ps_sslGut->e_smState));
				...
				case CERTIFICATE_VERIFY:
					...
					{
						LOG_ERR("%p| E_PENDACT_PROTOERR, state should be "
								"WAIT_CLIENT_CERT_VERIFY but state is %s",
								ps_sslCtx, sslDiag_getSMState(ps_sslGut->e_smState));
					...
					LOG2_HEX(ps_hsElem->ac_hsBuf, loc_getHashSizeByPrf(ps_sslCtx->s_secParams.e_prf));
					...
					if (ps_sslCtx->e_ver > E_TLS_1_1) {
						...
						case E_SSL_SIGN_RSA:
							...
							if (sslDerd_initDecCtx(&s_derdCtx, &s_sigOctStr) == SSL_DER_ASN1_UNDEF) {
								...
								*cwt_wDataLen = 1;
								if (memcmp(ps_hsElem->ac_hsBuf, ac_decSign, sz_decSignLen) == 0) {
									pc_wData[0] = 1;
								} else {
									pc_wData[0] = 0;
								}
						...
						CW_MEMCOPY(ps_hsElem->ac_hsBuf+VERIF_HASHSIZE, pc_hsBuff+6, cwt_len);
						*cwt_wDataLen = cwt_len + VERIF_HASHSIZE;
						CW_MEMCOPY(pc_wData, ps_hsElem->ac_hsBuf, *cwt_wDataLen);
				...
				case FINISHED:
				{
					...
					{
						LOG_ERR("%p| E_PENDACT_PROTOERR, state should be "
								"WAIT_FOR_xx_FINISH but state is %s",
								ps_sslCtx, sslDiag_getSMState(ps_sslGut->e_smState));
...

					{
								...
								ps_sslCtx->s_sslGut.c_verifyDataLen) != 0)
						{
							...
							LOG_INFO("%p| VerificationHashes in Finished "
									"not even!", ps_sslCtx);
						}
						else
						{
							if (ps_sslCtx->b_isCli == TRUE)
							{
								CW_MEMCOPY(ps_sslGut->ac_srvVerifyData,
										pc_hsHash,
										ps_sslGut->c_verifyDataLen);
							}
							else
							{
								CW_MEMCOPY(ps_sslGut->ac_cliVerifyData,
										pc_hsHash,
										ps_sslGut->c_verifyDataLen);
							}
						}
					...
					loc_hash(E_HASHOP_UPDATE, ps_sslCtx, pc_hsBuff, ps_sslGut->c_verifyDataLen + HS_HEADERLEN);
				...
				default:
					/* Failure */
					/* TG */
					LOG_ERR("%p| E_PENDACT_PROTOERR 0x%X unkown handshake "
							"type", ps_sslCtx,pc_hsBuff[0]);
					ps_sslCtx->e_lastError = E_SSL_ERROR_SM;
					action = E_PENDACT_PROTOERR;
		...
		e_sslRecType_t e_recType)
{
	...
	if ((ps_sslCtx->e_ver > E_TLS_1_0) && (ps_guts->e_txCipSpec != TLS_UNDEFINED)) {
		l_IVLen = ps_sslCtx->s_secParams.c_blockLen;
	}

	if (pc_rawTxt != pc_rec)
	{
		CW_MEMCOPY(pc_rec + l_IVLen, pc_rawTxt, cwt_rawTxtLen);
	} else if (l_IVLen > 0) {
	...
	case TLS_RSA_WITH_RC4_128_MD5:
		...
		len += loc_compMac(ps_sslCtx, pc_rec + len, cwt_maxLen,
				pc_rec,    len,          e_recType,
				SEND,      ps_secParams->e_hmacType);

		TIME_STAMP(TS_STREAM_ENCRYPT_BEGIN);

		if (ps_sslCtx->b_isCli == TRUE)
			cw_rc4(&ps_sslCtx->s_secParams.u_cliKey.cwt_cliRc4Ctx, pc_rec, pc_rec, len);
		else
			cw_rc4(&ps_sslCtx->s_secParams.u_srvKey.cwt_srvRc4Ctx, pc_rec, pc_rec, len);
		break;
	...
	case TLS_RSA_WITH_RC4_128_SHA:
		...
		if (ps_sslCtx->b_isCli == TRUE)
			cw_rc4(&ps_sslCtx->s_secParams.u_cliKey.cwt_cliRc4Ctx, pc_rec,
					pc_rec, len);
		else
			cw_rc4(&ps_sslCtx->s_secParams.u_srvKey.cwt_srvRc4Ctx, pc_rec,
					pc_rec, len);
	...
	case TLS_ECDHE_RSA_WITH_3DES_EDE_CBC_SHA:
		if (ps_sslCtx->b_isCli == TRUE) {
			cwt_cipCtx = &ps_sslCtx->s_secParams.u_cliKey.cwt_cli3DesCtx;
		} else {
			cwt_cipCtx = &ps_sslCtx->s_secParams.u_srvKey.cwt_srv3DesCtx;
		}
	...
	case TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA: //vpy
		if ((ps_sslCtx->b_isCli == TRUE) && (cwt_cipCtx == NULL)) {
			cwt_cipCtx = &ps_sslCtx->s_secParams.u_cliKey.cwt_cliAesCtx;
		} else if (cwt_cipCtx == NULL) {
			cwt_cipCtx = &ps_sslCtx->s_secParams.u_srvKey.cwt_srvAesCtx;
		}
		...
		len += loc_addPadding(ps_sslCtx,pc_rec, len, ps_sslCtx->s_secParams.c_blockLen);
...
static e_sslPendAct_t loc_smDecryptMacCheck(s_sslCtx_t * ps_sslCtx,
		uint8_t *pc_rawTxt, size_t *pcwt_rawTxtLen,
		uint8_t *pc_rec,  size_t cwt_recLen)
{
	size_t              cwt_hashLen;
	uint32_t            l_blkSize;
	...
	CW_MEMCOPY(ac_badRecBuf, pc_rec, cwt_recLen);
#endif
	...
	case TLS_RSA_WITH_RC4_128_MD5:
		...
		if (ps_sslCtx->b_isCli == TRUE)
			cw_rc4(&ps_sslCtx->s_secParams.u_srvKey.cwt_srvRc4Ctx,
					pc_rawTxt + REC_HEADERLEN, pc_rec + REC_HEADERLEN,
					cwt_recLen - REC_HEADERLEN);
		else
			cw_rc4(&ps_sslCtx->s_secParams.u_cliKey.cwt_cliRc4Ctx,
					pc_rawTxt + REC_HEADERLEN, pc_rec + REC_HEADERLEN,
					cwt_recLen - REC_HEADERLEN);

		TIME_STAMP(TS_STREAM_DECRYPT_END);

		cwt_hashLen += loc_compMac(ps_sslCtx,
				ac_hash, ps_sslCtx->s_secParams.c_hmacLen,
	...
	case TLS_RSA_WITH_RC4_128_SHA:
		...
		cwt_hashLen = loc_compMac(ps_sslCtx, ac_hash, ps_sslCtx->s_secParams.c_hmacLen,
				pc_rawTxt + REC_HEADERLEN,
				cwt_recLen - REC_HEADERLEN - ps_sslCtx->s_secParams.c_hmacLen,
				pc_rec[0], RCVR, ps_secParams->e_hmacType);
	...
	case TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA: //vpy
		...
		case TLS_ECDHE_RSA_WITH_3DES_EDE_CBC_SHA: //vpy
			...
			if (ps_sslCtx->b_isCli == TRUE)
				...
				cw_cbc_decrypt(&ps_sslCtx->s_secParams.u_srvKey.cwt_srv3DesCtx,
						pc_rawTxt + REC_HEADERLEN,
						pc_rec + REC_HEADERLEN,
						cwt_recLen - REC_HEADERLEN);
			else
				/* TODO: should catch encryption error */
				cw_cbc_decrypt(&ps_sslCtx->s_secParams.u_cliKey.cwt_cli3DesCtx,
						pc_rawTxt + REC_HEADERLEN,
						pc_rec + REC_HEADERLEN,
						cwt_recLen - REC_HEADERLEN);
		...
		default:
			...
			if (ps_sslCtx->b_isCli == TRUE)
				/* TODO: should catch encryption error */
				cw_cbc_decrypt(&ps_sslCtx->s_secParams.u_srvKey.cwt_srvAesCtx,
						pc_rawTxt + REC_HEADERLEN,
						pc_rec + REC_HEADERLEN,
						cwt_recLen - REC_HEADERLEN);
			else
				/* TODO: should catch encryption error */
				cw_cbc_decrypt(&ps_sslCtx->s_secParams.u_cliKey.cwt_cliAesCtx,
						pc_rawTxt + REC_HEADERLEN,
						pc_rec + REC_HEADERLEN,
						cwt_recLen - REC_HEADERLEN);
		...
		cwt_recLen -= loc_rmPadding(ps_sslCtx,
				pc_rawTxt + REC_HEADERLEN,
		...
		cwt_hashLen = loc_compMac(ps_sslCtx, ac_hash, ps_sslCtx->s_secParams.c_hmacLen,
				...
				cwt_recLen - REC_HEADERLEN -
				ps_sslCtx->s_secParams.c_hmacLen - l_blkSize,
				pc_rec[0], RCVR, ps_secParams->e_hmacType);
	...
	*pcwt_rawTxtLen = cwt_recLen - cwt_hashLen - l_blkSize;
	 ...
	 * values (#1384) */
	if (CW_MEMCMP(ac_hash, pc_rawTxt + *pcwt_rawTxtLen + l_blkSize, cwt_hashLen)
			!= 0)
	{
...
static e_sslPendAct_t loc_smLenVerCheck(s_sslCtx_t * ps_sslCtx)
{
	...
	if (ps_sslCtx->l_buffLen >= HS_HEADERLEN)
	{
		...
		{
			LOG_ERR("%p| The length received isn't equal to the "
					"expected record length", ps_sslCtx);
		...
		{
			LOG_ERR("%p| Received invalid record type %2X",
					ps_sslCtx, ps_sslCtx->ac_socBuf[0]);
...
int ssl_verifyHash(const uint8_t rac_verHash[], size_t cwt_verHashLen,
		...
		rpcw_rsaPubKey_t rpcw_pubKey)
{
	cw_bigNum_t *pcw_msg;
	cw_bigNum_t *pcw_sign;
	size_t   cwt_modLen;
	size_t   cwt_emLen;
	cw_rsaRet_t cwt_stat;
	...
	memset(pcw_msg, 0, sizeof(cw_bigNum_t));
	memset(pcw_sign, 0, sizeof(cw_bigNum_t));
#endif
	...
	if (CW_MEMCMP(ac_encMsg + cwt_emLen - VERIF_HASHSIZE, rac_verHash,
			VERIF_HASHSIZE) != 0)
		cwt_stat = CW_ERROR;
	else
		cwt_stat = CW_OK;
...
e_sslPendAct_t ssl_serverFSM(s_sslCtx_t *ps_sslCtx, e_sslPendAct_t e_event,
		uint8_t *pc_eventData,size_t cwt_eventDataLen,
		uint8_t *pc_actData, size_t *pcwt_actDataLen)
{
	...
	while (e_action > E_PENDACT_EXTERNAL_ACTION)
	{
		...
		case E_PENDACT_PROTORESPGEN:
			e_action = loc_protocolResp(ps_sslCtx, pc_wDataStart,
					&cwt_wDataLen, pc_rData, cwt_rDataLen);
		...
		case E_PENDACT_MAC_ENCRYPT_HANDSHAKE:
			...
			e_action = loc_smMacEncrypt(ps_sslCtx, pc_wDataStart,
					cwt_wDataLen, pc_wDataStart, &cwt_wDataLen,
					e_recordType);
			...
			(void) loc_buildRecordHeader(ps_sslCtx, pc_recStart,
					pc_wDataStart - pc_recStart - REC_HEADERLEN,
					e_recordType);
		...
		s_sslHsElem_t *ps_sslHsElem)
{
	...
	cw_rsa_publickey_init(&ps_sslHsElem->peerPubKey.key.rsaPub);

	ps_sslHsElem->cwt_hsBufLen = SSL_HANDSHAKE_BUFFER_SIZE;

	/* Initialise default security parameters */

	ps_sslCtx->s_secParams.e_prf = E_SSL_PRF_UNDEF;

	ps_sslCtx->s_secParams.e_kst = E_SSL_KST_UNDEF;

	ps_sslCtx->s_secParams.s_signAlg.c_hash = E_SSL_HASH_INVALID;

	ps_sslCtx->s_secParams.s_signAlg.c_sign = E_SSL_SIGN_INVALID;
